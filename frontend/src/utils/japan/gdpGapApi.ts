...
export async function fetchGDPGapData(forceRefresh = false): Promise<GDPGapResponse> {
  try {
    ...
    const url = `${API_BASE_URL}/api/japan/gdp-gap${params.toString() ? `?${params.toString()}` : ''}`
    ...
    console.error('Error fetching GDP Gap data:', error)
    return {
      data: [],
      latest: null,
      next_release: null,
