...
function CnBaiduMigrationChart() {
  ...
  const loadScreenshotUrl = async () => {
    try {
      setLoading(true)
      setError(null)
      const response = await fetchWithTimeout(`${API_BASE_URL}/api/china/baidu-migration-screenshot`, undefined, 30_000)
      if (!response.ok) {
  ...
  const handleRefresh = async () => {
    setRefreshing(true)
    try {
      const response = await fetchWithTimeout(`${API_BASE_URL}/api/china/baidu-migration-screenshot?force_refresh=true`, undefined, 90_000)
      if (!response.ok) {
            ...
            <div style={{ position: 'absolute', right: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
              <Tooltip title="データハンドブック">
                <QuestionCircleOutlined
                  onClick={() => openHandbook('cn-baidu-migration')}
...
export default withVisibility(CnBaiduMigrationChart, 'special')
