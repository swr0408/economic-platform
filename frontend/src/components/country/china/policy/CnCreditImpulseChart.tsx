...
function CnCreditImpulseChart() {
  ...
  const loadScreenshotUrl = async () => {
    try {
      setLoading(true)
      setError(null)
      const response = await fetchWithTimeout(`${API_BASE_URL}/api/china/credit-impulse-screenshot`, undefined, 30_000)
      if (!response.ok) {
  ...
  const handleRefresh = async () => {
    setRefreshing(true)
    try {
      const response = await fetchWithTimeout(`${API_BASE_URL}/api/china/credit-impulse-screenshot?force_refresh=true`, undefined, 90_000)
      if (!response.ok) {
