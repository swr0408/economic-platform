...
import type { DisposableIncomeData } from '../../../../hooks/useDashboardData'
...
interface DisposableIncomeChartProps {
  data: DisposableIncomeData | null
}

// カラー設定
const COLORS = {
  nominal: CHART_COLORS.orange,
  real: CHART_COLORS.purple,
}
...
export default function DisposableIncomeChart({ data }: DisposableIncomeChartProps) {
  ...
  if (data === null) {
    return <LoadingChart title="可処分所得（Disposable Personal Income）" />
  }

  if (!hasData) {
    return (
      <ChartContainer title="可処分所得（Disposable Personal Income）" showPeriodSelector={false} showDataSource={false}>
        <NoDataMessage />
      </ChartContainer>
    )
  }
        ...
        <Tabs
          ...
          items={[
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                    ...
                    <Tooltip title="比較ページを開く">
                      <Button
                        ...
                        onClick={() => window.open('/compare?s=disposable_income', '_blank')}
                      >
                        データ比較
                      </Button>
                    </Tooltip>
            ...
            },
            {
              ...
              label: 'マーケットインパクト',
              children: (
                <MarketImpactTab indicatorId="disposable_income" />
              ),
            },
          ]}
        />
