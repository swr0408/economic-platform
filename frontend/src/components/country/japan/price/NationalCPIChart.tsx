/**
 * Japan National CPI Chart Component
 * 全国消費者物価指数チャート
 *
 * データ項目:
 * - yoy: 前年同月比 (%)
...
// カラー設定
const COLORS = {
  all_yoy: CHART_COLORS.primary,
  all_mom: CHART_COLORS.primary,
...
export default function NationalCPIChart() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        setLoading(true)
        setError(null)
        const res = await fetchNationalCPIData()
        if (res.error) {
          setError(res.error)
        } else {
          setData(res)
        ...
        console.error('Error loading National CPI data:', err)
  ...
  // ローディング状態
  if (loading) {
    return <LoadingChart title="全国消費者物価指数（CPI）" />
  }

  // エラー状態
  if (error) {
    return (
      <ChartContainer title="全国消費者物価指数（CPI）" showPeriodSelector={false} showDataSource={false}>
  ...
  return (
    <div id="japan-national-cpi-chart">
      <ChartContainer
        title="全国消費者物価指数（CPI）"
        ...
        <Tabs
          ...
          items={[
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                    ...
                    <Tooltip title="比較ページを開く">
                      <Button
                        ...
                        onClick={() => window.open('/compare?s=japan_national_cpi_yoy', '_blank')}
                      >
                        データ比較
                      </Button>
                    </Tooltip>
            ...
            },
            {
              key: 'market-impact',
              label: 'マーケットインパクト',
              children: <MarketImpactTab indicatorId="jp_national_cpi" />,
            },
          ]}
        />
      </ChartContainer>
