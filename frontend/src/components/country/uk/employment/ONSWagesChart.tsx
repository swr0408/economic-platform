/**
 * UK ONS平均週給チャートコンポーネント
 *
 * データ:
 * - Total Pay (KAC3): 平均週給（賞与含む）前年比
 * - Regular Pay (KAI9): 平均週給（賞与除く）前年比
 *
 * データソース:
...
import type { ONSWagesData } from '../../../../hooks/useDashboardData'
import type { PeriodType } from '../../usa/common/useChartData'

interface ONSWagesChartProps {
  data: ONSWagesData | null
}
...
export default function ONSWagesChart({ data }: ONSWagesChartProps) {
  ...
  const handleCompare = useCallback(() => {
    window.open('/compare?s=uk_wages_total&s=uk_wages_regular', '_blank')
  }, [])

  if (data === null) {
    return <LoadingChart title="ONS平均週給（前年比）" />
  }
