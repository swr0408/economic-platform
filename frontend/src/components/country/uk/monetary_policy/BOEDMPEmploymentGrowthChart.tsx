...
interface BOEDMPEmploymentGrowthChartProps {
...
export default function BOEDMPEmploymentGrowthChart({ data }: BOEDMPEmploymentGrowthChartProps) {
  const chartData = useMemo(() => {
    const employmentData = data?.survey_data?.employment_growth

    if (!employmentData?.date?.length) {
      return []
    }

    const result: ChartDataPoint[] = employmentData.date.map((date, idx) => ({
      date,
      realised: employmentData.realised_3mo_avg?.[idx] ?? null,
      expected: employmentData.expected_3mo_avg?.[idx] ?? null,
    }))
