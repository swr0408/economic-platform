...
interface BOEDMPPriceGrowthChartProps {
...
export default function BOEDMPPriceGrowthChart({ data }: BOEDMPPriceGrowthChartProps) {
  const chartData = useMemo(() => {
    const priceData = data?.survey_data?.price_growth

    if (!priceData?.date?.length) {
      return []
    }

    const result: ChartDataPoint[] = priceData.date.map((date, idx) => ({
      date,
      realised: priceData.realised_3mo_avg?.[idx] ?? null,
      expected: priceData.expected_3mo_avg?.[idx] ?? null,
    }))
