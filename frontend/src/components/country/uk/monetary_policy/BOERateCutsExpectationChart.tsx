...
interface BOERateCutsExpectationChartProps {
  title?: string
}
...
function BOERateCutsExpectationChart({
  title = 'BOE利上げ・利下げ期待（2026年）'
}: BOERateCutsExpectationChartProps) {
  ...
  const loadScreenshotUrls = async () => {
    try {
      setLoading(true)
      setError(null)
      const response = await fetchWithTimeout(`${API_BASE_URL}/api/uk/boe-rate-cuts-screenshot`, undefined, 30_000)
      if (!response.ok) {
  ...
  const handleRefresh = async () => {
    ...
    setError(null)
    try {
      const response = await fetchWithTimeout(`${API_BASE_URL}/api/uk/boe-rate-cuts-screenshot?force_refresh=true`, undefined, 120_000)
      if (!response.ok) {
          ...
          <div style={{ textAlign: 'center' }}>
            ...
            {data?.yearend_url ? (
              <Image
                ...
                src={`${API_BASE_URL}/api/uk/boe-rate-cuts-screenshot/yearend?t=${imageKey}`}
                alt="BOE Yearend Rate Expectations"
          ...
          <div style={{ textAlign: 'center' }}>
            ...
            {data?.rate_cuts_url ? (
              <Image
                ...
                src={`${API_BASE_URL}/api/uk/boe-rate-cuts-screenshot/rate-cuts?t=${imageKey}`}
                alt="BOE Rate Cuts Expectations"
                style={{ maxWidth: '100%', height: 'auto' }}
                preview={true}
                placeholder={
                  <div style={{ padding: '40px', background: '#f5f5f5' }}>
...
export default withVisibility(BOERateCutsExpectationChart, 'special')
