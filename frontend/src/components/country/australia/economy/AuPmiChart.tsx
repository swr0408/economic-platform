/**
 * オーストラリア S&P Global PMI チャートコンポーネント
 *
 * 製造業PMI、サービス業PMI、総合PMIを表示
 *
 * データソース:
 * - S&P Global (旧 Judo Bank)
 *
 * 発表スケジュール:
 * - 速報値: 毎月第3週（23日前後）
 * - 確報値: 毎月第1週（1日前後）
...
import type { AuPmiData } from '../../../../hooks/useDashboardData'
...
interface AuPmiChartProps {
  data: AuPmiData | null
}

export default function AuPmiChart({ data }: AuPmiChartProps) {
  ...
  if (data === null) {
    return <LoadingChart title="S&P Global PMI（オーストラリア）" />
  }

  if (!hasData) {
    return (
      <ChartContainer title="S&P Global PMI（オーストラリア）" showPeriodSelector={false} showDataSource={false}>
        <NoDataMessage />
      </ChartContainer>
    )
  }
        ...
        <Tabs
          ...
          items={[
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                    ...
                    <Tooltip title="比較ページを開く（S&P PMI）">
                      <Button
                        ...
                        onClick={() => window.open('/compare?s=au_man_pmi&s=au_ser_pmi&s=au_com_pmi', '_blank')}
                      >
                        データ比較
                      </Button>
                    </Tooltip>
            ...
            },
            {
              ...
              label: 'マーケットインパクト',
              children: (
                <MarketImpactTab indicatorId="au_man_pmi" />
              ),
            },
          ]}
        />
