...
import type { NzUnemploymentRateData } from '../../../../hooks/useDashboardData'

interface NzUnemploymentRateChartProps {
  data: NzUnemploymentRateData | null
}
        ...
        <Tabs
          ...
          items={[
              ...
              children: (
                <>
                  <div style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', marginBottom: 8 }}>
                    <Tooltip title="比較ページを開く">
                      <Button
                        ...
                        onClick={() => window.open('/compare?s=nz_unemployment_rate', '_blank')}
                      >
                        データ比較
                      </Button>
                    </Tooltip>
                  ...
                  <StandardLineChart
                    data={filteredData}
                    lines={[
                      { dataKey: 'value', color: CHART_COLOR, name: '失業率' },
                    ]}
                    yAxisFormatter={(v) => `${v}%`}
                    tooltipValueFormatter={(v) => `${v.toFixed(1)}%`}
                    yDomain={['dataMin - 0.25', 'dataMax + 0.25']}
            ...
            },
            {
              ...
              label: 'マーケットインパクト',
              children: (
                <MarketImpactTab indicatorId="nz_unemployment_rate" />
              ),
            },
          ]}
        />
