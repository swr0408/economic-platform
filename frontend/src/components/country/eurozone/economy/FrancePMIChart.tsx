/**
 * フランス HCOB PMIチャートコンポーネント
 *
 * 製造業PMI、サービス業PMI、総合PMIを表示
 *
 * データソース:
 * - HCOB (formerly S&P Global)
...
import type { FrancePMIData } from '../../../../hooks/useDashboardData'
...
interface FrancePMIChartProps {
  data: FrancePMIData | null
}
...
export default function FrancePMIChart({ data }: FrancePMIChartProps) {
  ...
  if (data === null) {
    return <LoadingChart title="HCOB PMI（フランス）" />
  }

  if (!hasData) {
    return (
      <ChartContainer title="HCOB PMI（フランス）" showPeriodSelector={false} showDataSource={false}>
        <NoDataMessage />
      </ChartContainer>
    )
  }
        ...
        <Tabs
          ...
          items={[
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                    ...
                    <Tooltip title="比較ページを開く（HCOB PMI）">
                      <Button
                        ...
                        onClick={() => window.open('/compare?s=fr_hcob_manufacturing_pmi&s=fr_hcob_services_pmi&s=fr_hcob_composite_pmi', '_blank')}
                      >
                        データ比較
                      </Button>
                    </Tooltip>
            ...
            },
            {
              ...
              label: 'マーケットインパクト',
              children: (
                <MarketImpactTab indicatorId="france_pmi" />
              ),
            },
          ]}
        />
