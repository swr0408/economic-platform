 ...
 * ECB鉱工業生産チャートコンポーネント
 *
 * ECB Data APIからユーロ圏鉱工業生産データを取得し、表示
 *
 * データ:
 * - 鉱工業生産指数（営業日調整）: WDA Index 2021=100
 * - 前月比変化率 (MoM)
 * - 前年比変化率 (YoY)
 *
 * データソース:
...
import type { ECBProductionData } from '../../../../hooks/useDashboardData'

interface ECBProductionChartProps {
  data: ECBProductionData | null
}
...
export default function ECBProductionChart({ data }: ECBProductionChartProps) {
  ...
  if (data === null) {
    return <LoadingChart title="鉱工業生産（ユーロ圏）" />
  }

  if (!hasData) {
    return (
      <ChartContainer title="鉱工業生産（ユーロ圏）" showPeriodSelector={false} showDataSource={false}>
        <NoDataMessage />
      </ChartContainer>
    )
  }
  ...
  return (
    <div id="ecb-production-chart">
      <ChartContainer
        title="鉱工業生産（ユーロ圏）"
        showPeriodSelector={false}
        dataSource="Eurostat"
        handbookId="ecb-production"
        ...
        <Tabs
          ...
          items={[
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                    ...
                    <Tooltip title="比較ページを開く">
                      <Button
                        ...
                        onClick={() => window.open('/compare?s=ecb_production_yoy', '_blank')}
                      >
                        データ比較
                      </Button>
                    </Tooltip>
            ...
            },
            {
              ...
              label: 'マーケットインパクト',
              children: (
                <MarketImpactTab indicatorId="ecb_production" />
              ),
            },
          ]}
        />
