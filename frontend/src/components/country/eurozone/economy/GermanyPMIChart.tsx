...
import type { GermanyPMIData } from '../../../../hooks/useDashboardData'
...
interface GermanyPMIChartProps {
  data: GermanyPMIData | null
}
...
export default function GermanyPMIChart({ data }: GermanyPMIChartProps) {
  ...
  if (data === null) {
    return <LoadingChart title="S&P Global PMI（ドイツ）" />
  }

  if (!hasData) {
    return (
      <ChartContainer title="S&P Global PMI（ドイツ）" showPeriodSelector={false} showDataSource={false}>
        <NoDataMessage />
      </ChartContainer>
    )
  }
        ...
        <Tabs
          ...
          items={[
              ...
              children: (
                <>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                    ...
                    <Tooltip title="比較ページを開く（S&P Global PMI）">
                      <Button
                        icon={<AreaChartOutlined />}
                        onClick={() => window.open('/compare?s=de_sp_manufacturing_pmi&s=de_sp_services_pmi&s=de_sp_composite_pmi', '_blank')}
                      >
                        データ比較
                      </Button>
                    </Tooltip>
            ...
            },
            {
              ...
              label: 'マーケットインパクト',
              children: (
                <MarketImpactTab indicatorId="germany_pmi" />
              ),
            },
          ]}
        />
