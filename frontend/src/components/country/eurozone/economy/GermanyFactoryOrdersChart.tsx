...
// 統一されたビューモード定義
type ViewMode = 'mom_chart' | 'mom_table' | 'yoy_chart' | 'index_chart'

const VIEW_MODE_OPTIONS: { mode: ViewMode; label: string }[] = [
  { mode: 'mom_chart', label: '前月比' },
  { mode: 'mom_table', label: '前月比（テーブル）' },
  { mode: 'yoy_chart', label: '前年比' },
  { mode: 'index_chart', label: '指数' },
]
...
export default function GermanyFactoryOrdersChart({ data }: GermanyFactoryOrdersChartProps) {
  ...
  // 現在のデータタイプを判定
  const dataType = viewMode === 'index_chart' ? 'index' : viewMode.startsWith('yoy') ? 'yoy' : 'mom'
