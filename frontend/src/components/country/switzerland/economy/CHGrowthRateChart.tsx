/**
 * スイスGDP成長率チャートコンポーネント
 *
 * SECOから四半期GDP成長率データを取得し、前期比・前年比・年率を表示
 *
 * データ:
...
import type { CHGrowthRateData } from '../../../../hooks/useDashboardData'

interface CHGrowthRateChartProps {
  data: CHGrowthRateData | null
}
...
// 指標種別
type DataKind = 'yoy' | 'qoq' | 'annualized'
...
export default function CHGrowthRateChart({ data }: CHGrowthRateChartProps) {
  ...
  const currentValue = useMemo(() => {
    ...
    if (dataKind === 'annualized') return latest.annualized
    return latest.qoq
  }, [latest, dataKind])
  ...
  const currentColor = useMemo(() => {
    ...
    if (dataKind === 'annualized') return COLORS.annualized
    return COLORS.qoq
  }, [dataKind])
  ...
  const getCompareId = () => {
    if (dataKind === 'yoy') return 'ch_growth_rate_yoy'
    if (dataKind === 'annualized') return 'ch_growth_rate_annualized'
    return 'ch_growth_rate_qoq'
  }

  const getChartTitle = () => {
    if (dataKind === 'yoy') return 'GDP成長率（前年比）'
    if (dataKind === 'annualized') return 'GDP成長率（年率）'
    return 'GDP成長率（前期比）'
  }

  return (
    <div id="ch-growth-rate-chart">
      <ChartContainer
        title="GDP成長率"
        ...
        <Tabs
          ...
          items={[
                ...
                <>
                  ...
                  {dataKind === 'annualized' && (
                    <StandardBarChart
                      data={filteredData}
                      bars={[
                        { dataKey: 'annualized', color: COLORS.annualized, name: 'GDP成長率（年率）' },
                      ]}
                      yAxisFormatter={(v) => `${v}%`}
                      xAxisFormatter={formatQuarterLabel}
                    />
            ...
            },
            {
              ...
              label: 'マーケットインパクト',
              children: (
                <MarketImpactTab indicatorId="ch_growth_rate" />
              ),
            },
          ]}
        />
