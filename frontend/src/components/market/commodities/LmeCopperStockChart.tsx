...
const COLOR_STOCK = '#f97316'     // Orange - warehouse stock
const COLOR_CASH = '#06b6d4'      // Cyan - cash settlement
const COLOR_3M = '#10b981'        // Green - 3-month forward

interface LmeItem {
  date: string
  cash_usd: number | null
  three_month_usd: number | null
...
type SeriesKey = 'stock_tonnes' | 'cash_usd' | 'three_month_usd'
...
function ChartTooltip({ active, payload, label, hiddenSeries }: {
  ...
  const dp = payload[0]?.payload as LmeItem | undefined
  if (!dp) return null
  ...
  return (
    <div style={{
      ...
      {!hiddenSeries.has('stock_tonnes') && dp.stock_tonnes != null && (
        <div style={{ color: COLOR_STOCK, fontSize: 13, marginBottom: 3 }}>
          在庫: {dp.stock_tonnes.toLocaleString()} tonnes
        </div>
      )}
      {!hiddenSeries.has('cash_usd') && dp.cash_usd != null && (
        <div style={{ color: COLOR_CASH, fontSize: 13, marginBottom: 3 }}>
          Cash: ${dp.cash_usd.toLocaleString(undefined, { maximumFractionDigits: 1 })} /t
        </div>
      )}
      {!hiddenSeries.has('three_month_usd') && dp.three_month_usd != null && (
        <div style={{ color: COLOR_3M, fontSize: 13, marginBottom: 3 }}>
          3M: ${dp.three_month_usd.toLocaleString(undefined, { maximumFractionDigits: 1 })} /t
        </div>
      )}
    </div>
  )
...
export default function LmeCopperStockChart() {
  ...
  const { data: response } = useQuery<LmeResponse>({
    queryKey: ['lme-copper-stock'],
    queryFn: async () => {
      const res = await axios.get('/api/market/lme-copper-stock')
      return res.data
    },
  ...
  // Monthly averages for stock_tonnes
  ...
  return (
    <ChartContainer
      title="LME Copper Warehouse Stock"
      dataSource="LME"
      sourceUrl="https://www.lme.com/market-data/reports-and-data/warehouse-and-stocks-reports"
    ...
    >
        ...
        <div style={LATEST_VALUE_BOX_STYLE}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
              <span style={{ color: TEXT_COLORS.secondary, fontSize: 11 }}>在庫</span>
              <span style={{ color: COLOR_STOCK, fontSize: 20, fontWeight: 700 }} className="tabular-nums">
                {latest.stock_tonnes?.toLocaleString()}
              </span>
              <span style={{ color: TEXT_COLORS.secondary, fontSize: 11 }}>tonnes</span>
            </div>

            <div style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
              <span style={{ color: TEXT_COLORS.secondary, fontSize: 11 }}>Cash</span>
              <span style={{ color: COLOR_CASH, fontSize: 15, fontWeight: 600 }} className="tabular-nums">
                ${latest.cash_usd?.toLocaleString(undefined, { maximumFractionDigits: 1 })}
              </span>
              <span style={{ color: TEXT_COLORS.secondary, fontSize: 11 }}>/t</span>
            </div>

            <div style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
              <span style={{ color: TEXT_COLORS.secondary, fontSize: 11 }}>3M</span>
              <span style={{ color: COLOR_3M, fontSize: 15, fontWeight: 600 }} className="tabular-nums">
                ${latest.three_month_usd?.toLocaleString(undefined, { maximumFractionDigits: 1 })}
              </span>
              <span style={{ color: TEXT_COLORS.secondary, fontSize: 11 }}>/t</span>
            </div>
            ...
            {latestCopper && (
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
                ...
                <span style={{ color: '#a78bfa', fontSize: 15, fontWeight: 600 }} className="tabular-nums">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
        ...
        <Tooltip title="比較ページを開く">
          <Button
            ...
            size="small"
            onClick={() => window.open('/compare?s=lme_copper_stock', '_blank')}
          >
            データ比較
          </Button>
        </Tooltip>
          ...
          <ResponsiveContainer width="100%" height={400}>
            <ComposedChart data={filteredData} margin={CHART_MARGIN}>
              ...
              <YAxis
                ...
                width={50}
                label={{ value: 'Stock (tonnes)', angle: -90, position: 'insideLeft', offset: -8, fill: COLOR_STOCK, fontSize: 10 }}
              />
              <YAxis
                yAxisId="right"
                orientation="right"
                ...
                axisLine={{ stroke: DARK_THEME.axisLine }}
                tickLine={{ stroke: DARK_THEME.axisLine }}
                tick={{ fill: COLOR_CASH, fontSize: 10 }}
                width={70}
                label={{ value: 'USD/tonne', angle: 90, position: 'insideRight', offset: -4, fill: COLOR_CASH, fontSize: 10 }}
              />
              ...
              <Line
                ...
                strokeWidth={1.5}
                dot={false}
                hide={hiddenSeries.has('stock_tonnes')}
                connectNulls
              ...
              <Line
                ...
                type="monotone"
                dataKey="cash_usd"
                name="Cash Settlement (USD/t)"
                stroke={COLOR_CASH}
                strokeWidth={1.5}
                dot={false}
                hide={hiddenSeries.has('cash_usd')}
                connectNulls
                isAnimationActive={false}
              />
              <Line
                yAxisId="right"
                type="monotone"
                dataKey="three_month_usd"
                name="3-Month Forward (USD/t)"
                stroke={COLOR_3M}
                strokeWidth={1.5}
                strokeDasharray="4 3"
                dot={false}
                hide={hiddenSeries.has('three_month_usd')}
                connectNulls
                isAnimationActive={false}
              />
            </ComposedChart>
      ...
      {viewMode === 'mom' && displayMode === 'heatmap' && (
        <MonthlyTable
          ...
          decimals={2}
          showLegend
          helperText="※ LME銅在庫 前月比（日次データの月平均から算出, 単位: %）"
        />
      )}
    </ChartContainer>
  )
