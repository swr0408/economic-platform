...
const COLOR_TOTAL = '#94a3b8'       // Silver/slate
...
interface SilverResponse {
  data: SilverItem[]
  latest: SilverItem | null
  latest_silver: { date: string; close: number } | null
...
function ChartTooltip({ active, payload, label }: {
  ...
  const dp = payload[0]?.payload as SilverItem | undefined
  if (!dp) return null
...
export default function ComexSilverStockChart() {
  ...
  const { data: response } = useQuery<SilverResponse>({
    queryKey: ['comex-silver-stock'],
    queryFn: async () => {
      const res = await axios.get('/api/market/comex-silver-stock')
      return res.data
    },
  ...
  const latestSilver = response?.latest_silver

  return (
    <ChartContainer
      title="COMEX Total Silver Inventory"
    ...
    >
      ...
      {latest && (
        <div style={LATEST_VALUE_BOX_STYLE}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
            ...
            {latestSilver && (
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
                <span style={{ color: TEXT_COLORS.secondary, fontSize: 11 }}>先物(SI=F)</span>
                <span style={{ color: '#e2e8f0', fontSize: 15, fontWeight: 600 }} className="tabular-nums">
                  ${latestSilver.close.toFixed(2)}
                </span>
                <span style={{ color: TEXT_COLORS.secondary, fontSize: 11 }}>/oz</span>
              </div>
            )}
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
        ...
        <Tooltip title="比較ページを開く">
          <Button
            ...
            size="small"
            onClick={() => window.open('/compare?s=comex_silver_stock', '_blank')}
          >
            データ比較
          </Button>
        </Tooltip>
      ...
      {viewMode === 'mom' && displayMode === 'heatmap' && (
        <MonthlyTable
          ...
          decimals={2}
          showLegend
          helperText="※ COMEX銀在庫 前月比（日次データの月平均から算出, 単位: %）"
        />
      )}
    </ChartContainer>
  )
