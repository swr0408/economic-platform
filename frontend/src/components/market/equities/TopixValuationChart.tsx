...
// Colors
const COLOR_TOPIX = '#f97316'             // Orange - TOPIX
...
const COLOR_RATIO = '#ec4899'             // Pink - Yield ratio
...
function ChartTooltip({ active, payload, label, viewMode }: {
  ...
  if (viewMode === 'raw') {
    items.push({ label: 'TOPIX', value: Number(dp.close).toLocaleString(undefined, { maximumFractionDigits: 2 }), color: COLOR_TOPIX })
...
export default function TopixValuationChart() {
  ...
  const { data: response } = useQuery<ValuationResponse>({
    queryKey: ['topix-valuation'],
    queryFn: async () => {
      const res = await axios.get('/api/market/topix-valuation')
      return res.data
    },
  ...
  return (
    ...
    >
        ...
        <div style={LATEST_VALUE_BOX_STYLE}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
              <span style={{ color: TEXT_COLORS.secondary, fontSize: 11 }}>TOPIX</span>
              <span style={{ color: COLOR_TOPIX, fontSize: 20, fontWeight: 700 }} className="tabular-nums">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
        ...
        <Tooltip title="比較ページを開く">
          <Button
            ...
            size="small"
            onClick={() => window.open('/compare?s=topix_valuation', '_blank')}
          >
            データ比較
          </Button>
        </Tooltip>
      ...
      {/* ===== View 1: Raw (TOPIX + Forward EPS + Forward PE) ===== */}
      {viewMode === 'raw' && (
        <>
          ...
          <ResponsiveContainer width="100%" height={400}>
            <ComposedChart data={filteredData} margin={CHART_MARGIN}>
              ...
              <YAxis
                ...
                axisLine={{ stroke: DARK_THEME.axisLine }}
                tickLine={{ stroke: DARK_THEME.axisLine }}
                tick={{ fill: COLOR_TOPIX, fontSize: 10 }}
                width={55}
                label={{ value: 'TOPIX', angle: -90, position: 'insideLeft', offset: -5, fill: COLOR_TOPIX, fontSize: 10 }}
              />
              <YAxis
                yAxisId="eps"
                orientation="right"
          ...
          <div style={{ marginTop: 8 }}>
            {momSubView === 'pe' && (
              <MonthlyTable
                ...
                decimals={2}
                showLegend
                helperText="※ 月次PERデータから前月比を算出（単位: %）"
              />
            )}
            {momSubView === 'eps' && (
              <MonthlyTable
          ...
          <ResponsiveContainer width="100%" height={400}>
            <ComposedChart data={yieldChartData} margin={CHART_MARGIN}>
              ...
              <YAxis
                ...
                domain={['dataMin * 0.9', 'dataMax * 1.05']}
                tickFormatter={(v: number) => v.toLocaleString(undefined, { maximumFractionDigits: 0 })}
                axisLine={{ stroke: COLOR_TOPIX, strokeDasharray: '4 3' }}
