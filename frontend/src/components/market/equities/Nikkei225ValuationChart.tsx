...
const COLOR_YIELD_10Y = '#3b82f6'       // Blue - JGB 10Y
...
function ChartTooltip({ active, payload, label, viewMode }: {
  ...
  if (viewMode === 'raw') {
    items.push({ label: '日経平均', value: Number(dp.close).toLocaleString(undefined, { maximumFractionDigits: 0 }), color: COLOR_NIKKEI })
...
type MomSubView = 'pe' | 'eps'
const MOM_SUB_OPTIONS = [
  { mode: 'pe' as MomSubView, label: '予想PER' },
  { mode: 'eps' as MomSubView, label: '予想EPS' },
]

export default function Nikkei225ValuationChart() {
  const [currentPeriod, setCurrentPeriod] = useState<PeriodValue>(5)
  const [viewMode, setViewMode] = useState<ViewMode>('raw')
  const [momSubView, setMomSubView] = useState<MomSubView>('pe')
  ...
  const { data: response } = useQuery<ValuationResponse>({
    queryKey: ['nikkei225-valuation'],
    queryFn: async () => {
      const res = await axios.get('/api/market/nikkei225-valuation')
      return res.data
    },
  ...
  // Monthly table for MoM heatmap - PE
  const momPeHeatmapData = useMemo(() => {
  ...
  // Monthly table for MoM heatmap - EPS
  const momEpsHeatmapData = useMemo(() => {
    if (!response?.monthly_table) return { years: [] as number[], monthlyData: {} as Record<number, Record<number, number | null>> }
    const currentYear = new Date().getFullYear()
    const startYear = currentYear - 9
    const years: number[] = []
    ...
    for (const year of years) {
      ...
      for (let m = 0; m < 12; m++) {
        const ym = `${year}-${String(m + 1).padStart(2, '0')}`
        const row = response.monthly_table.find(r => r.date === ym)
        monthlyData[year][m] = row?.forward_eps_mom ?? null
      }
    }
    return { years, monthlyData }
  }, [response?.monthly_table])
  ...
  return (
    ...
    >
        ...
        <div style={LATEST_VALUE_BOX_STYLE}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
              <span style={{ color: TEXT_COLORS.secondary, fontSize: 11 }}>日経平均</span>
              <span style={{ color: COLOR_NIKKEI, fontSize: 20, fontWeight: 700 }} className="tabular-nums">
            ...
            {latest.yield_10y != null && (
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
                <span style={{ color: TEXT_COLORS.secondary, fontSize: 11 }}>JGB 10Y</span>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
        ...
        <Tooltip title="比較ページを開く">
          <Button
            ...
            size="small"
            onClick={() => window.open('/compare?s=nikkei225_valuation', '_blank')}
          >
            データ比較
          </Button>
        </Tooltip>
      ...
      {/* ===== View 1: Raw (日経平均 + Forward EPS + Forward PE) ===== */}
      {viewMode === 'raw' && (
        <>
          ...
          <ResponsiveContainer width="100%" height={400}>
            <ComposedChart data={filteredData} margin={CHART_MARGIN}>
              ...
              <YAxis
                ...
                axisLine={{ stroke: DARK_THEME.axisLine }}
                tickLine={{ stroke: DARK_THEME.axisLine }}
                tick={{ fill: COLOR_NIKKEI, fontSize: 10 }}
                width={60}
                label={{ value: '日経平均', angle: -90, position: 'insideLeft', offset: -5, fill: COLOR_NIKKEI, fontSize: 10 }}
              />
              <YAxis
                yAxisId="eps"
                orientation="right"
      ...
      {/* ===== View 2: YoY (Forward PE / Forward EPS / Earnings Yield YoY) ===== */}
      {viewMode === 'yoy' && (
        <>
          <PeriodSelector onPeriodChange={setCurrentPeriod} selectedPeriod={currentPeriod} />
          <ResponsiveContainer width="100%" height={400}>
      ...
      {/* ===== View 3: MoM Heatmap (PE / EPS toggle) ===== */}
      {viewMode === 'mom' && (
        <>
          <ViewModeButtonGroup
            options={MOM_SUB_OPTIONS}
            currentMode={momSubView}
            onChange={(m) => setMomSubView(m as MomSubView)}
          />
          <div style={{ marginTop: 8 }}>
            {momSubView === 'pe' && (
              <MonthlyTable
                data={momPeHeatmapData}
                decimals={2}
                showLegend
      ...
      {/* ===== View 4: Yield Spread (Earnings Yield vs JGB 10Y + Spread + 日経平均) ===== */}
          ...
          <ResponsiveContainer width="100%" height={400}>
            <ComposedChart data={yieldChartData} margin={CHART_MARGIN}>
              ...
              <YAxis
                ...
                domain={['dataMin * 0.9', 'dataMax * 1.05']}
                tickFormatter={(v: number) => v.toLocaleString(undefined, { maximumFractionDigits: 0 })}
                axisLine={{ stroke: COLOR_NIKKEI, strokeDasharray: '4 3' }}
      ...
      {/* ===== View 5: Yield Ratio (Earnings Yield vs JGB 10Y + Ratio + 日経平均) ===== */}
