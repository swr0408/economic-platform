...
// Colors
const COLOR_NDX = '#8b5cf6'         // Purple - Nasdaq100
...
export default function Nasdaq100ValuationChart() {
  ...
  const { data: response } = useQuery<ValuationResponse>({
    ...
    queryFn: async () => {
      const res = await axios.get('/api/market/nasdaq100-valuation')
      return res.data
    },
  ...
  return (
    ...
    >
        ...
        <div style={LATEST_VALUE_BOX_STYLE}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
              <span style={{ color: TEXT_COLORS.secondary, fontSize: 11 }}>Nasdaq 100</span>
              <span style={{ color: COLOR_NDX, fontSize: 20, fontWeight: 700 }} className="tabular-nums">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
        ...
        <Tooltip title="比較ページを開く">
          <Button
            ...
            size="small"
            onClick={() => window.open('/compare?s=nasdaq100_valuation', '_blank')}
          >
            データ比較
          </Button>
        </Tooltip>
      ...
      {/* ===== Chart 1: Raw (Nasdaq100 + Forward EPS + Forward PE) ===== */}
      {viewMode === 'raw' && (
        <>
          ...
          <ResponsiveContainer width="100%" height={400}>
            <ComposedChart data={filteredData} margin={CHART_MARGIN}>
              ...
              <YAxis
                ...
                width={55}
                label={{ value: 'Nasdaq 100', angle: -90, position: 'insideLeft', offset: -5, fill: COLOR_NDX, fontSize: 10 }}
              />
              <YAxis
                yAxisId="eps"
                orientation="right"
          ...
          <ResponsiveContainer width="100%" height={400}>
            <ComposedChart data={yieldChartData} margin={CHART_MARGIN}>
              ...
              <YAxis
                ...
                domain={['dataMin * 0.9', 'dataMax * 1.05']}
                tickFormatter={(v: number) => v.toLocaleString(undefined, { maximumFractionDigits: 0 })}
                axisLine={{ stroke: COLOR_NDX, strokeDasharray: '4 3' }}
