...
const COLOR_CUSHING = '#22c55e'   // Green
const COLOR_OIL = '#ef4444'       // Red (inverted)

interface CushingItem {
  date: string
  value: number | null
  yoy: number | null
}

interface CushingResponse {
  data: CushingItem[]
  latest: CushingItem | null
...
interface MergedItem extends CushingItem {
    ...
    <div style={{
      ...
      {viewMode === 'raw' ? (
        !hiddenSeries.has('value') && dp.value != null && (
          <div style={{ color: COLOR_CUSHING, fontSize: 13, marginBottom: 3 }}>
            クッシング在庫: {formatKb(dp.value)}
          </div>
        )
      ) : (
        !hiddenSeries.has('yoy') && dp.yoy != null && (
          <div style={{ color: COLOR_CUSHING, fontSize: 13, marginBottom: 3 }}>
            クッシング在庫 YoY: {formatYoy(dp.yoy)}
          </div>
        )
      )}
...
export default function CushingInventoryChart() {
  ...
  const { data: response } = useQuery<CushingResponse>({
    queryKey: ['cushing-inventory'],
    queryFn: async () => {
      const res = await axios.get('/api/market/cushing-inventory')
      return res.data
    },
    ...
    >
      ...
      <div style={LATEST_VALUE_BOX_STYLE}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
          ...
          <div>
            <span style={{ fontSize: 12, color: TEXT_COLORS.secondary }}>クッシング在庫: </span>
            <span style={{ fontSize: 20, fontWeight: 'bold', color: COLOR_CUSHING }}>
      ...
      <Tabs
        ...
        items={[
              ...
              <>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                  ...
                  <Tooltip title="比較ページを開く">
                    <Button
                      ...
                      onClick={() => window.open('/compare?s=us_cushing_inventory', '_blank')}
                    >
                      データ比較
                    </Button>
                  </Tooltip>
                  ...
                  <>
                    ...
                    <ResponsiveContainer width="100%" height={400}>
                      <ComposedChart data={filteredData} margin={CHART_MARGIN}>
                        ...
                        <YAxis yAxisId="left" domain={['dataMin * 0.9', 'dataMax * 1.05']} tickFormatter={(v: number) => `${(v / 1000).toFixed(0)}K`} stroke={COLOR_CUSHING} tick={{ fill: COLOR_CUSHING, fontSize: 11 }} width={55} />
                        ...
                        <Line yAxisId="left" type="monotone" dataKey="value" name="クッシング在庫 (千bbl)" stroke={COLOR_CUSHING} strokeWidth={2} dot={false} hide={hiddenSeries.has('value')} connectNulls isAnimationActive={false} />
                  ...
                  <>
                    ...
                    <ResponsiveContainer width="100%" height={400}>
                      <ComposedChart data={filteredData} margin={CHART_MARGIN}>
                        <CartesianGrid strokeDasharray="3 3" stroke={DARK_THEME.gridLine} fill={DARK_THEME.chartBg} />
                        <XAxis dataKey="date" tickFormatter={(v) => formatDayLabel(v)} stroke={DARK_THEME.axisLine} tick={{ fill: DARK_THEME.textSecondary, fontSize: 11 }} minTickGap={40} />
                        <YAxis yAxisId="left" domain={['auto', 'auto']} tickFormatter={(v: number) => `${v.toFixed(1)}%`} stroke={COLOR_CUSHING} tick={{ fill: COLOR_CUSHING, fontSize: 11 }} width={55} />
                        <YAxis yAxisId="oil" orientation="right" reversed domain={['dataMin * 0.9', 'dataMax * 1.1']} tickFormatter={(v: number) => `$${v.toFixed(0)}`} stroke={COLOR_OIL} tick={{ fill: COLOR_OIL, fontSize: 10 }} width={50} axisLine={{ stroke: COLOR_OIL, strokeDasharray: '4 3' }} />
                        <RechartsTooltip content={<ChartTooltip hiddenSeries={hiddenSeries} viewMode={viewMode} />} />
                        <ReferenceLine yAxisId="left" y={0} stroke={DARK_THEME.axisLine} strokeDasharray="3 3" />
                        <Legend onClick={(e) => handleLegendClick(e.dataKey as SeriesKey)} wrapperStyle={{ cursor: 'pointer' }} formatter={(value: string, entry: any) => (<span style={{ color: hiddenSeries.has(entry.dataKey as SeriesKey) ? '#64748b' : entry.color, fontSize: 12 }}>{value}</span>)} />
                        <Line yAxisId="left" type="monotone" dataKey="yoy" name="クッシング在庫 YoY %" stroke={COLOR_CUSHING} strokeWidth={2} dot={false} hide={hiddenSeries.has('yoy')} connectNulls isAnimationActive={false} />
                        <Line yAxisId="oil" type="monotone" dataKey="oil_price" name="WTI原油 (USD/bbl, 反転)" stroke={COLOR_OIL} strokeWidth={1.5} strokeDasharray="4 3" dot={false} hide={hiddenSeries.has('oil_price')} connectNulls isAnimationActive={false} />
                      </ComposedChart>
                    </ResponsiveContainer>
                  </>
                    ...
                    <ResponsiveContainer width="100%" height={400}>
                      <ComposedChart data={filteredMomChartData} margin={CHART_MARGIN}>
                        ...
                        <RechartsTooltip
                          ...
                          formatter={(value: number) => [`${value >= 0 ? '+' : ''}${value.toFixed(2)}%`, 'クッシング在庫 MoM']}
                        />
                        <ReferenceLine y={0} stroke={DARK_THEME.axisLine} strokeDasharray="3 3" />
                        <Bar dataKey="mom" name="クッシング在庫 前月比 (%)" fill={COLOR_CUSHING} isAnimationActive={false} />
                      </ComposedChart>
                    </ResponsiveContainer>
                  </>
                )}
                ...
                {viewMode === 'mom' && displayMode === 'heatmap' && (
                  <MonthlyTable
                    ...
                    decimals={2}
                    showLegend
                    helperText="※ クッシング在庫 前月比（週次データの月平均から算出, 単位: %）"
                  />
                )}
              </>
            ),
          },
          {
            ...
            label: 'マーケットインパクト',
            children: (
              <MarketImpactTab indicatorId="us_cushing_inventory" />
            ),
          },
        ]}
      />
