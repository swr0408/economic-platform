...
const COLOR_GASOLINE = '#22c55e'    // Green
const COLOR_REFINERY = '#a855f7'    // Purple
const COLOR_OIL = '#ef4444'         // Red

interface DataItem {
  date: string
  gasoline: number | null
  refinery_util: number | null
}
...
interface MergedItem extends DataItem {
  oil_price: number | null
  gasoline_yoy: number | null
}
...
type SeriesKey = 'gasoline' | 'refinery_util' | 'oil_price' | 'gasoline_yoy'
...
function ChartTooltip({ active, payload, label, hiddenSeries, viewMode }: {
  ...
  return (
    <div style={{
      ...
      {viewMode === 'raw' && (
        <>
          {!hiddenSeries.has('gasoline') && dp.gasoline != null && (
            <div style={{ color: COLOR_GASOLINE, fontSize: 13, marginBottom: 3 }}>
              ガソリン在庫: {(dp.gasoline / 1000).toLocaleString(undefined, { maximumFractionDigits: 1 })} 百万bbl
            </div>
          )}
          {!hiddenSeries.has('refinery_util') && dp.refinery_util != null && (
            <div style={{ color: COLOR_REFINERY, fontSize: 13, marginBottom: 3 }}>
              製油稼働率: {dp.refinery_util.toFixed(1)}%
            </div>
          )}
        </>
      )}
      {viewMode === 'yoy' && !hiddenSeries.has('gasoline_yoy') && dp.gasoline_yoy != null && (
        <div style={{ color: COLOR_GASOLINE, fontSize: 13, marginBottom: 3 }}>
          ガソリン在庫 YoY: {dp.gasoline_yoy >= 0 ? '+' : ''}{dp.gasoline_yoy.toFixed(2)}%
        </div>
      )}
...
export default function UsGasolineRefineryChart() {
  ...
  const { data: response } = useQuery<DataResponse>({
    queryKey: ['gasoline-refinery'],
    queryFn: async () => {
      const res = await axios.get('/api/market/gasoline-refinery')
      return res.data
    },
  ...
  const mergedData = useMemo(() => {
    ...
    // Build date→gasoline map for YoY
    const gasolineMap = new Map<string, number>()
    for (const item of response.data) {
      if (item.gasoline != null) gasolineMap.set(item.date, item.gasoline)
    }
    ...
    return response.data.map(item => {
      ...
      if (oilMap.has(item.date)) {
        ...
        for (let offset = 1; offset <= 3; offset++) {
          ...
          if (oilMap.has(beforeStr)) { oilPrice = oilMap.get(beforeStr)!; break }
          const after = new Date(d)
          after.setDate(after.getDate() + offset)
          const afterStr = after.toISOString().slice(0, 10)
          if (oilMap.has(afterStr)) { oilPrice = oilMap.get(afterStr)!; break }
        }
      }

      // YoY for gasoline
      let gasolineYoy: number | null = null
      if (item.gasoline != null) {
        const d = new Date(item.date)
        const lastYear = new Date(d)
        lastYear.setFullYear(lastYear.getFullYear() - 1)
        // Search ±7 days around last year same date
        for (let offset = 0; offset <= 7; offset++) {
          for (const dir of [0, -1, 1]) {
            if (offset === 0 && dir !== 0) continue
            const check = new Date(lastYear)
            check.setDate(check.getDate() + offset * dir)
            const checkStr = check.toISOString().slice(0, 10)
            const prevVal = gasolineMap.get(checkStr)
            if (prevVal != null && prevVal !== 0) {
              gasolineYoy = ((item.gasoline - prevVal) / prevVal) * 100
              break
            }
          }
          if (gasolineYoy != null) break
        }
      }

      return { ...item, oil_price: oilPrice, gasoline_yoy: gasolineYoy }
    })
  }, [response, marketData])
    ...
    >
      ...
      <div style={LATEST_VALUE_BOX_STYLE}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
          ...
          <div>
            <span style={{ fontSize: 12, color: TEXT_COLORS.secondary }}>ガソリン在庫: </span>
            <span style={{ fontSize: 20, fontWeight: 'bold', color: COLOR_GASOLINE }}>
              {latest?.gasoline != null ? `${(latest.gasoline / 1000).toLocaleString(undefined, { maximumFractionDigits: 1 })} 百万bbl` : '—'}
            </span>
          </div>
          {latest?.refinery_util != null && (
            <div>
              <span style={{ fontSize: 12, color: TEXT_COLORS.secondary }}>製油稼働率: </span>
              <span style={{ fontSize: 16, fontWeight: 'bold', color: COLOR_REFINERY }}>
                {latest.refinery_util.toFixed(1)}%
              </span>
            </div>
          )}
          {latestOil && (
      ...
      <Tabs
        ...
        items={[
              ...
              <>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                  ...
                  <Tooltip title="比較ページを開く">
                    <Button
                      ...
                      onClick={() => window.open('/compare?s=us_gasoline_inventories', '_blank')}
                    >
                      データ比較
                    </Button>
                  </Tooltip>
                  ...
                  <>
                    ...
                    <ResponsiveContainer width="100%" height={400}>
                      <ComposedChart data={filteredData} margin={CHART_MARGIN}>
                        ...
                        <YAxis yAxisId="left" domain={['dataMin * 0.95', 'dataMax * 1.02']} tickFormatter={(v: number) => `${(v / 1000).toFixed(0)}M`} stroke={COLOR_GASOLINE} tick={{ fill: COLOR_GASOLINE, fontSize: 11 }} width={55} />
                        <YAxis yAxisId="refinery" orientation="right" reversed domain={['dataMin * 0.95', 'dataMax * 1.02']} tickFormatter={(v: number) => `${v.toFixed(0)}%`} stroke={COLOR_REFINERY} tick={{ fill: COLOR_REFINERY, fontSize: 10 }} width={45} axisLine={{ stroke: COLOR_REFINERY }} />
                  ...
                  <>
                    ...
                    <ResponsiveContainer width="100%" height={400}>
                      <ComposedChart data={filteredData} margin={CHART_MARGIN}>
                        <CartesianGrid strokeDasharray="3 3" stroke={DARK_THEME.gridLine} fill={DARK_THEME.chartBg} />
                        <XAxis dataKey="date" tickFormatter={(v) => formatDayLabel(v)} stroke={DARK_THEME.axisLine} tick={{ fill: DARK_THEME.textSecondary, fontSize: 11 }} minTickGap={40} />
                        <YAxis yAxisId="left" domain={['auto', 'auto']} tickFormatter={(v: number) => `${v.toFixed(1)}%`} stroke={COLOR_GASOLINE} tick={{ fill: COLOR_GASOLINE, fontSize: 11 }} width={55} />
                        <YAxis yAxisId="oil" orientation="right" reversed domain={['dataMin * 0.9', 'dataMax * 1.1']} tickFormatter={(v: number) => `$${v.toFixed(0)}`} stroke={COLOR_OIL} tick={{ fill: COLOR_OIL, fontSize: 10 }} width={45} axisLine={{ stroke: COLOR_OIL, strokeDasharray: '4 3' }} />
                        <RechartsTooltip content={<ChartTooltip hiddenSeries={hiddenSeries} viewMode={viewMode} />} />
                ...
                {viewMode === 'mom' && displayMode === 'heatmap' && (
                  <MonthlyTable
                    ...
                    decimals={2}
                    showLegend
                    helperText="※ ガソリン在庫 前月比（週次データの月平均から算出, 単位: %）"
                  />
                )}
              </>
            ),
          },
          {
            ...
            label: 'マーケットインパクト',
            children: (
              <MarketImpactTab indicatorId="us_gasoline_inventories_refinery_utilization_rate" />
            ),
          },
        ]}
      />
