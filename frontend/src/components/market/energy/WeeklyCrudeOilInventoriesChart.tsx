...
const COLOR_TOTAL = '#3b82f6'     // Blue
const COLOR_EX_SPR = '#22c55e'    // Green
const COLOR_SPR = '#a855f7'       // Purple
...
type SeriesKey = 'total' | 'ex_spr' | 'spr' | 'oil_price' | 'total_yoy' | 'ex_spr_yoy' | 'spr_yoy'
...
function ChartTooltip({ active, payload, label, hiddenSeries, viewMode }: {
  ...
  const formatMb = (v: number) => `${(v / 1000).toLocaleString(undefined, { maximumFractionDigits: 1 })} 百万bbl`
  ...
  return (
    <div style={{
      ...
      {viewMode === 'raw' ? (
        <>
          {!hiddenSeries.has('total') && dp.total != null && (
            <div style={{ color: COLOR_TOTAL, fontSize: 13, marginBottom: 3 }}>
              合計 (SPR含): {formatMb(dp.total)}
            </div>
          )}
          {!hiddenSeries.has('ex_spr') && dp.ex_spr != null && (
            <div style={{ color: COLOR_EX_SPR, fontSize: 13, marginBottom: 3 }}>
              商業在庫: {formatMb(dp.ex_spr)}
            </div>
          )}
          {!hiddenSeries.has('spr') && dp.spr != null && (
            <div style={{ color: COLOR_SPR, fontSize: 13, marginBottom: 3 }}>
              SPR: {formatMb(dp.spr)}
            </div>
          )}
        </>
        ...
        <>
          {!hiddenSeries.has('total_yoy') && dp.total_yoy != null && (
            <div style={{ color: COLOR_TOTAL, fontSize: 13, marginBottom: 3 }}>
              合計 YoY: {formatYoy(dp.total_yoy)}
            </div>
          )}
          {!hiddenSeries.has('ex_spr_yoy') && dp.ex_spr_yoy != null && (
            <div style={{ color: COLOR_EX_SPR, fontSize: 13, marginBottom: 3 }}>
              商業在庫 YoY: {formatYoy(dp.ex_spr_yoy)}
            </div>
          )}
          {!hiddenSeries.has('spr_yoy') && dp.spr_yoy != null && (
            <div style={{ color: COLOR_SPR, fontSize: 13, marginBottom: 3 }}>
              SPR YoY: {formatYoy(dp.spr_yoy)}
            </div>
          )}
        </>
      )}
...
export default function WeeklyCrudeOilInventoriesChart() {
  ...
  const { data: response } = useQuery<InventoryResponse>({
    queryKey: ['weekly-crude-oil-inventories'],
    queryFn: async () => {
      const res = await axios.get('/api/market/weekly-crude-oil-inventories')
      return res.data
    },
  ...
  const mergedData = useMemo(() => {
    ...
    // Build weekly oil price map (use closest date)
    ...
    return response.data.map(item => {
      // Find oil price for the same week (try exact date, then ±3 days)
  ...
  // MoM heatmap (weekly → monthly average → MoM %)
  const momHeatmapData = useMemo(() => {
    ...
    for (const year of years) {
      monthlyData[year] = {}
      for (let m = 0; m < 12; m++) {
        const curKey = `${year}-${String(m).padStart(2, '0')}`
        const curVal = monthAvg[curKey]
  ...
  // Series config by view mode
  const rawSeries = [
    { key: 'total' as const, name: '合計 (千bbl)', color: COLOR_TOTAL, yAxisId: 'left' },
    { key: 'ex_spr' as const, name: '商業在庫 (千bbl)', color: COLOR_EX_SPR, yAxisId: 'right1' },
    { key: 'spr' as const, name: 'SPR (千bbl)', color: COLOR_SPR, yAxisId: 'right1' },
  ]
  const yoySeries = [
    { key: 'total_yoy' as const, name: '合計 YoY %', color: COLOR_TOTAL, yAxisId: 'left' },
    { key: 'ex_spr_yoy' as const, name: '商業在庫 YoY %', color: COLOR_EX_SPR, yAxisId: 'left' },
    { key: 'spr_yoy' as const, name: 'SPR YoY %', color: COLOR_SPR, yAxisId: 'left' },
  ]
    ...
    >
      ...
      <div style={LATEST_VALUE_BOX_STYLE}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
          ...
          <div>
            <span style={{ fontSize: 12, color: TEXT_COLORS.secondary }}>商業在庫: </span>
            <span style={{ fontSize: 20, fontWeight: 'bold', color: COLOR_EX_SPR }}>
              {latest?.ex_spr != null ? `${(latest.ex_spr / 1000).toLocaleString(undefined, { maximumFractionDigits: 1 })} 百万bbl` : '—'}
            </span>
          </div>
          {latest?.spr != null && (
            <div>
              <span style={{ fontSize: 12, color: TEXT_COLORS.secondary }}>SPR: </span>
              <span style={{ fontSize: 16, fontWeight: 'bold', color: COLOR_SPR }}>
                {(latest.spr / 1000).toLocaleString(undefined, { maximumFractionDigits: 1 })} 百万bbl
              </span>
            </div>
          )}
              ...
              <>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                  ...
                  <Tooltip title="比較ページを開く">
                    <Button
                      ...
                      onClick={() => window.open('/compare?s=weekly_crude_oil_inventories_ex_spr', '_blank')}
                    >
                      データ比較
                    </Button>
                  </Tooltip>
                  ...
                  <>
                    ...
                    <ResponsiveContainer width="100%" height={400}>
                      <ComposedChart data={filteredData} margin={CHART_MARGIN}>
                        ...
                        <YAxis yAxisId="left" domain={['dataMin * 0.95', 'dataMax * 1.02']} tickFormatter={(v: number) => `${(v / 1000).toFixed(0)}M`} stroke={COLOR_TOTAL} tick={{ fill: COLOR_TOTAL, fontSize: 11 }} width={55} />
                        <YAxis yAxisId="right1" orientation="right" domain={['dataMin * 0.9', 'dataMax * 1.05']} tickFormatter={(v: number) => `${(v / 1000).toFixed(0)}M`} stroke={COLOR_EX_SPR} tick={{ fill: COLOR_EX_SPR, fontSize: 10 }} width={45} axisLine={{ stroke: COLOR_EX_SPR }} />
                  ...
                  <>
                    ...
                    <ResponsiveContainer width="100%" height={400}>
                      <ComposedChart data={filteredData} margin={CHART_MARGIN}>
                        <CartesianGrid strokeDasharray="3 3" stroke={DARK_THEME.gridLine} fill={DARK_THEME.chartBg} />
                        <XAxis dataKey="date" tickFormatter={(v) => formatDayLabel(v)} stroke={DARK_THEME.axisLine} tick={{ fill: DARK_THEME.textSecondary, fontSize: 11 }} minTickGap={40} />
                        <YAxis yAxisId="left" domain={['auto', 'auto']} tickFormatter={(v: number) => `${v.toFixed(1)}%`} stroke={COLOR_TOTAL} tick={{ fill: COLOR_TOTAL, fontSize: 11 }} width={55} />
                        <YAxis yAxisId="oil" orientation="right" reversed domain={['dataMin * 0.9', 'dataMax * 1.1']} tickFormatter={(v: number) => `$${v.toFixed(0)}`} stroke={COLOR_OIL} tick={{ fill: COLOR_OIL, fontSize: 10 }} width={45} axisLine={{ stroke: COLOR_OIL, strokeDasharray: '4 3' }} />
                        <RechartsTooltip content={<ChartTooltip hiddenSeries={hiddenSeries} viewMode={viewMode} />} />
                ...
                {viewMode === 'mom' && displayMode === 'heatmap' && (
                  <MonthlyTable
                    ...
                    decimals={2}
                    showLegend
                    helperText="※ 商業在庫 前月比（週次データの月平均から算出, 単位: %）"
                  />
                )}
              </>
            ),
