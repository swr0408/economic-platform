...
const COLOR_STORAGE = '#22c55e'   // Green
const COLOR_GAS = '#f59e0b'       // Amber - natural gas price (inverted)
...
interface MergedItem extends StorageItem {
...
type SeriesKey = 'value' | 'yoy' | 'gas_price'
...
function ChartTooltip({ active, payload, label, hiddenSeries, viewMode }: {
  ...
  const formatBcf = (v: number) => `${v.toLocaleString()} BCF`
      ...
      {viewMode === 'raw' ? (
        !hiddenSeries.has('value') && dp.value != null && (
          <div style={{ color: COLOR_STORAGE, fontSize: 13, marginBottom: 3 }}>
            天然ガス貯蔵量: {formatBcf(dp.value)}
          </div>
        )
      ) : (
        !hiddenSeries.has('yoy') && dp.yoy != null && (
          <div style={{ color: COLOR_STORAGE, fontSize: 13, marginBottom: 3 }}>
            天然ガス貯蔵量 YoY: {formatYoy(dp.yoy)}
          </div>
        )
      )}
      {!hiddenSeries.has('gas_price') && dp.gas_price != null && (
        <div style={{ color: COLOR_GAS, fontSize: 13, marginBottom: 3 }}>
          天然ガス: ${dp.gas_price.toLocaleString(undefined, { maximumFractionDigits: 2 })} /MMBtu
        </div>
      )}
    </div>
  )
...
export default function UsNaturalGasStorageChart() {
  ...
  const { data: response } = useQuery<StorageResponse>({
    ...
    queryFn: async () => {
      const res = await axios.get('/api/market/natural-gas-storage')
      return res.data
    },
  ...
  const { data: marketData } = useMarketBatchData(['natural_gas'])
  ...
  const mergedData = useMemo(() => {
    ...
    return response.data.map(item => {
      ...
      if (gasMap.has(item.date)) {
        gasPrice = gasMap.get(item.date)!
      } else {
        ...
        for (let offset = 1; offset <= 3; offset++) {
          ...
          if (gasMap.has(beforeStr)) {
            gasPrice = gasMap.get(beforeStr)!
            break
          }
          const after = new Date(d)
          after.setDate(after.getDate() + offset)
          const afterStr = after.toISOString().slice(0, 10)
          if (gasMap.has(afterStr)) {
            gasPrice = gasMap.get(afterStr)!
            break
          }
        }
      }

      return { ...item, gas_price: gasPrice }
    })
  }, [response, marketData])
  ...
  const latestGas = marketData?.natural_gas?.latest
    ...
    >
      ...
      <div style={LATEST_VALUE_BOX_STYLE}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
          ...
          <div>
            <span style={{ fontSize: 12, color: TEXT_COLORS.secondary }}>貯蔵量: </span>
            <span style={{ fontSize: 20, fontWeight: 'bold', color: COLOR_STORAGE }}>
              {latest?.value != null ? `${latest.value.toLocaleString()} BCF` : '—'}
            </span>
          </div>
          {latest?.yoy != null && (
            <div>
          ...
          {latestGas && (
            <div>
              ...
              <span style={{ fontSize: 16, fontWeight: 'bold', color: COLOR_GAS }}>
                ${latestGas.close.toLocaleString(undefined, { maximumFractionDigits: 2 })}
              </span>
            </div>
          )}
        </div>
      ...
      <Tabs
        ...
        items={[
              ...
              <>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                  ...
                  <Tooltip title="比較ページを開く">
                    <Button
                      ...
                      onClick={() => window.open('/compare?s=us_natural_gas_storage', '_blank')}
                    >
                      データ比較
                    </Button>
                  </Tooltip>
                  ...
                  <>
                    ...
                    <ResponsiveContainer width="100%" height={400}>
                      <ComposedChart data={filteredData} margin={CHART_MARGIN}>
                        ...
                        <YAxis yAxisId="left" domain={['dataMin * 0.9', 'dataMax * 1.05']} tickFormatter={(v: number) => `${(v / 1000).toFixed(1)}K`} stroke={COLOR_STORAGE} tick={{ fill: COLOR_STORAGE, fontSize: 11 }} width={55} />
                        <YAxis yAxisId="gas" orientation="right" reversed domain={['dataMin * 0.8', 'dataMax * 1.2']} tickFormatter={(v: number) => `$${v.toFixed(1)}`} stroke={COLOR_GAS} tick={{ fill: COLOR_GAS, fontSize: 10 }} width={50} axisLine={{ stroke: COLOR_GAS, strokeDasharray: '4 3' }} />
                        ...
                        <Line yAxisId="left" type="monotone" dataKey="value" name="天然ガス貯蔵量 (BCF)" stroke={COLOR_STORAGE} strokeWidth={2} dot={false} hide={hiddenSeries.has('value')} connectNulls isAnimationActive={false} />
                        <Line yAxisId="gas" type="monotone" dataKey="gas_price" name="天然ガス (USD/MMBtu, 反転)" stroke={COLOR_GAS} strokeWidth={1.5} strokeDasharray="4 3" dot={false} hide={hiddenSeries.has('gas_price')} connectNulls isAnimationActive={false} />
                      </ComposedChart>
                    </ResponsiveContainer>
                  </>
                )}
                  ...
                  <>
                    ...
                    <ResponsiveContainer width="100%" height={400}>
                      <ComposedChart data={filteredData} margin={CHART_MARGIN}>
                        <CartesianGrid strokeDasharray="3 3" stroke={DARK_THEME.gridLine} fill={DARK_THEME.chartBg} />
                        <XAxis dataKey="date" tickFormatter={(v) => formatDayLabel(v)} stroke={DARK_THEME.axisLine} tick={{ fill: DARK_THEME.textSecondary, fontSize: 11 }} minTickGap={40} />
                        <YAxis yAxisId="left" domain={['auto', 'auto']} tickFormatter={(v: number) => `${v.toFixed(1)}%`} stroke={COLOR_STORAGE} tick={{ fill: COLOR_STORAGE, fontSize: 11 }} width={55} />
                        <YAxis yAxisId="gas" orientation="right" reversed domain={['dataMin * 0.8', 'dataMax * 1.2']} tickFormatter={(v: number) => `$${v.toFixed(1)}`} stroke={COLOR_GAS} tick={{ fill: COLOR_GAS, fontSize: 10 }} width={50} axisLine={{ stroke: COLOR_GAS, strokeDasharray: '4 3' }} />
                        <RechartsTooltip content={<ChartTooltip hiddenSeries={hiddenSeries} viewMode={viewMode} />} />
                        <ReferenceLine yAxisId="left" y={0} stroke={DARK_THEME.axisLine} strokeDasharray="3 3" />
                        <Legend onClick={(e) => handleLegendClick(e.dataKey as SeriesKey)} wrapperStyle={{ cursor: 'pointer' }} formatter={(value: string, entry: any) => (<span style={{ color: hiddenSeries.has(entry.dataKey as SeriesKey) ? '#64748b' : entry.color, fontSize: 12 }}>{value}</span>)} />
                        <Line yAxisId="left" type="monotone" dataKey="yoy" name="天然ガス貯蔵量 YoY %" stroke={COLOR_STORAGE} strokeWidth={2} dot={false} hide={hiddenSeries.has('yoy')} connectNulls isAnimationActive={false} />
                        <Line yAxisId="gas" type="monotone" dataKey="gas_price" name="天然ガス (USD/MMBtu, 反転)" stroke={COLOR_GAS} strokeWidth={1.5} strokeDasharray="4 3" dot={false} hide={hiddenSeries.has('gas_price')} connectNulls isAnimationActive={false} />
                      </ComposedChart>
                    </ResponsiveContainer>
                  </>
                    ...
                    <ResponsiveContainer width="100%" height={400}>
                      <ComposedChart data={filteredMomChartData} margin={CHART_MARGIN}>
                        ...
                        <RechartsTooltip
                          ...
                          formatter={(value: number) => [`${value >= 0 ? '+' : ''}${value.toFixed(2)}%`, '天然ガス貯蔵量 MoM']}
                        />
                        <ReferenceLine y={0} stroke={DARK_THEME.axisLine} strokeDasharray="3 3" />
                        <Bar dataKey="mom" name="天然ガス貯蔵量 前月比 (%)" fill={COLOR_STORAGE} isAnimationActive={false} />
                      </ComposedChart>
                    </ResponsiveContainer>
                  </>
                )}
                ...
                {viewMode === 'mom' && displayMode === 'heatmap' && (
                  <MonthlyTable
                    ...
                    decimals={2}
                    showLegend
                    helperText="※ 天然ガス貯蔵量 前月比（週次データの月平均から算出, 単位: %）"
                  />
                )}
              </>
            ),
          },
          {
            ...
            label: 'マーケットインパクト',
            children: (
              <MarketImpactTab indicatorId="us_natural_gas_storage" />
            ),
          },
        ]}
      />
