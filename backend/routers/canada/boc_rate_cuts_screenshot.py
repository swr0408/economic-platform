"""
BOC Rate Cuts Expectation Screenshot APIルーター

エンドポイント:
- GET /api/canada/boc-rate-cuts-screenshot - スクリーンショットURLを取得
- GET /api/canada/boc-rate-cuts-screenshot/refresh - スクリーンショットを強制更新
- GET /api/canada/boc-rate-cuts-screenshot/cache/status - キャッシュ状態を取得
"""
...
router = APIRouter(
    prefix="/api/canada/boc-rate-cuts-screenshot",
    tags=["canada", "boc-rate-cuts-screenshot"]
)
...
async def get_screenshot_urls(
    ...
    """
    if force_refresh:
        result = boc_rate_cuts_screenshot_service.capture_all_screenshots(force_refresh=True)
        return {
    ...
    urls = boc_rate_cuts_screenshot_service.get_screenshot_urls()
    return {
...
async def get_yearend_screenshot():
    ...
    if not YEAREND_SCREENSHOT_PATH.exists():
        # キャッシュがない場合は取得を試みる
        boc_rate_cuts_screenshot_service.capture_all_screenshots()

    if YEAREND_SCREENSHOT_PATH.exists():
        return FileResponse(
            YEAREND_SCREENSHOT_PATH,
            media_type="image/png",
            filename="boc_yearend_rate_expectations.png"
        )
...
async def get_rate_cuts_screenshot():
    ...
    if RATE_CUTS_SCREENSHOT_PATH.exists():
        return FileResponse(
            RATE_CUTS_SCREENSHOT_PATH,
            media_type="image/png",
            filename="boc_rate_cuts_expectations.png"
        )
...
async def get_cache_status():
    ...
    Returns:
        キャッシュ状態情報
    """
    return boc_rate_cuts_screenshot_service.get_cache_status()
...
async def invalidate_cache():
    ...
    Returns:
        無効化結果
    """
    success = boc_rate_cuts_screenshot_service.invalidate_cache()
    return {
        "success": success,
