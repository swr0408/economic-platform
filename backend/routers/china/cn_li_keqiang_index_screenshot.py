"""
中国 李克強指数（Li Keqiang Index）Screenshot APIルーター

エンドポイント:
- GET /api/china/li-keqiang-index-screenshot - スクリーンショットURLを取得
- GET /api/china/li-keqiang-index-screenshot/image - スクリーンショット画像を取得
- POST /api/china/li-keqiang-index-screenshot/refresh - スクリーンショットを強制更新
...
try:
    from backend.services.china.cn_li_keqiang_index_screenshot_service import (
        cn_li_keqiang_index_screenshot_service,
        SCREENSHOT_PATH,
    )
except ImportError:
    from services.china.cn_li_keqiang_index_screenshot_service import (
        cn_li_keqiang_index_screenshot_service,
        SCREENSHOT_PATH,
    )
...
router = APIRouter(
    prefix="/api/china/li-keqiang-index-screenshot",
    tags=["china", "li-keqiang-index-screenshot"]
)
...
async def get_screenshot_image():
    ...
    if SCREENSHOT_PATH.exists():
        return FileResponse(
            ...
            filename="cn_li_keqiang_index.png"
        )
...
def get_cache_status():
    """キャッシュの状態を取得"""
    return cn_li_keqiang_index_screenshot_service.get_cache_status()


@router.post("/cache/invalidate")
def invalidate_cache():
    """キャッシュを無効化"""
    success = cn_li_keqiang_index_screenshot_service.invalidate_cache()
    return {
        "success": success,
