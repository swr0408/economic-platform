"""
台湾製造業PMI（S&P Global）サービス
DBからS&P Global Manufacturing PMI（台湾）データを取得

指標:
- S&P Global Manufacturing PMI (Taiwan): 台湾製造業PMI
...
注意事項:
- country='TW'でフィルタリング（他国のPMIを除外）
- 速報値と確報値がある場合は確報値で上書き
"""
import json
...
DATA_CACHE_FILE = CACHE_DIR / "taiwan_manufacturing_pmi_cache.json"


class TaiwanManufacturingPmiService:
    """台湾製造業PMI（S&P Global）サービス"""

    DATA_CACHE_KEY = "global:taiwan_manufacturing_pmi:data"
    ECONALPHA_ID = "taiwan_pmi_outlook"

    # DBクエリ用のイベントパターン
    EVENT_PATTERNS = ["S&P Global Manufacturing PMI"]
    ...
    def get_data(self, force_refresh: bool = False) -> Dict[str, Any]:
        ...
        if data:
            ...
            metadata = {
                "source": "S&P Global",
                "indicator": "Taiwan Manufacturing PMI (S&P Global)",
                "description": "台湾製造業PMI（S&P Global）",
                "unit": "index",
                "frequency": "monthly",
            }
    ...
    def _load_from_db(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            with SessionLocal() as session:
                # country='TW'でフィルタリングして他国のPMIを除外
                ...
                print(f"[TaiwanManufacturingPMI] Loaded {len(result)} records from DB")
                return result

        except Exception as e:
            print(f"[TaiwanManufacturingPMI] Error loading from DB: {e}")
            import traceback
            traceback.print_exc()
            return []
    ...
    def _load_file_cache(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            print(f"[TaiwanManufacturingPMI] Failed to load file cache: {e}")
            return None
    ...
    def _save_file_cache(self, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            print(f"[TaiwanManufacturingPMI] Failed to save file cache: {e}")
...
# シングルトンインスタンス
taiwan_manufacturing_pmi_service = TaiwanManufacturingPmiService()
