...
指標:
- Export Orders YoY (Taiwan): 台湾輸出受注（前年比）

データソース:
- DB: economic_calendar_events（CSV過去データ + FMP蓄積データ）
- 原典: MOEA (Ministry of Economic Affairs, Taiwan)
...
DATA_CACHE_FILE = CACHE_DIR / "taiwan_export_orders_cache.json"


class TaiwanExportOrdersService:
    """台湾輸出受注（前年比）サービス"""

    DATA_CACHE_KEY = "global:taiwan_export_orders:data"
    ECONALPHA_ID = "taiwan_export_orders"

    # DBクエリ用のイベントパターン
    EVENT_PATTERNS = ["Export Orders YoY"]
    ...
    def get_data(self, force_refresh: bool = False) -> Dict[str, Any]:
        ...
        if data:
            ...
            metadata = {
                "source": "MOEA (Ministry of Economic Affairs, Taiwan)",
                "indicator": "Taiwan Export Orders YoY",
                "description": "台湾輸出受注（前年比）",
                "unit": "%",
                "frequency": "monthly",
            }
        ...
        try:
            ...
            with SessionLocal() as session:
                ...
                for row in rows:
                    ...
                    if dt_utc:
                        # イベント名から対象月を抽出（例: "Export Orders YoY (Oct)"）
                ...
                print(f"[TaiwanExportOrders] Loaded {len(result)} records from DB")
                return result

        except Exception as e:
            print(f"[TaiwanExportOrders] Error loading from DB: {e}")
            import traceback
            traceback.print_exc()
            return []
    ...
    def _load_file_cache(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            print(f"[TaiwanExportOrders] Failed to load file cache: {e}")
            return None
    ...
    def _save_file_cache(self, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            print(f"[TaiwanExportOrders] Failed to save file cache: {e}")
...
# シングルトンインスタンス
taiwan_export_orders_service = TaiwanExportOrdersService()
