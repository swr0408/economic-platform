"""
SNB当座預金（Sight Deposits with the SNB）サービス
SNB Data Portalから当座預金データを取得

指標:
- スイス国立銀行への当座預金（Sight deposits with the SNB）

データソース:
- SNB Data Portal: https://data.snb.ch/api/warehouse/cube/BSTA.SNB.MIRE_U.MIR.AAK.GGU/data/csv/en
...
DATA_CACHE_FILE = CACHE_DIR / "snb_sight_deposits_cache.json"


class SNBSightDepositsService:
    """SNB当座預金サービス"""

    DATA_CACHE_KEY = "switzerland:snb_sight_deposits:data"

    # SNB Data Portal API URL
    # BSTA.SNB.MIRE_U.MIR.AAK.GGU = Sight deposits with the SNB
    DATA_SOURCE_URL = "https://data.snb.ch/api/warehouse/cube/BSTA.SNB.MIRE_U.MIR.AAK.GGU/data/csv/en"

    def __init__(self):
        self._publishing_date: Optional[datetime] = None

    def get_sight_deposits_data(self, force_refresh: bool = False) -> Dict[str, Any]:
        """SNB当座預金データを取得"""
        # Redisキャッシュチェック
        ...
        if result:
            ...
            cache_payload = {
                ...
                "metadata": {
                    ...
                    "indicator": "Sight Deposits with the SNB",
                    "description": "スイス国立銀行への当座預金",
                    "unit": "CHF (thousands)",
                },
    ...
    def _load_from_api(self) -> tuple[List[Dict[str, Any]], Optional[datetime]]:
        ...
        """
        try:
            print(f"[SNBSightDeposits] Fetching data from: {self.DATA_SOURCE_URL}")
                ...
                if 'PublishingDate' in line:
                    ...
                    if len(parts) >= 2:
                        ...
                        try:
                            ...
                            print(f"[SNBSightDeposits] PublishingDate: {publishing_date}")
                        except ValueError as e:
                            print(f"[SNBSightDeposits] Error parsing PublishingDate '{date_str}': {e}")
                    break

            # データ部分をDataFrameに読み込み
            df = pd.read_csv(io.StringIO(csv_content), sep=';', skiprows=2)

            print(f"[SNBSightDeposits] CSV columns: {df.columns.tolist()}")
            print(f"[SNBSightDeposits] Total rows: {len(df)}")
            ...
            for _, row in df.iterrows():
                ...
                if pd.notna(date_str) and pd.notna(value):
                    try:
                        ...
                        print(f"[SNBSightDeposits] Error parsing row: {e}")
                        continue

            # 日付でソート
            result.sort(key=lambda x: x["date"])

            print(f"[SNBSightDeposits] Loaded {len(result)} records")
            if result:
                print(f"[SNBSightDeposits] Date range: {result[0]['date']} to {result[-1]['date']}")
                print(f"[SNBSightDeposits] Latest: value={result[-1].get('value')}")

            return result, publishing_date

        except Exception as e:
            print(f"[SNBSightDeposits] Error loading data: {e}")
            import traceback
            traceback.print_exc()
            return [], None
    ...
    def _calculate_next_release(self) -> Optional[str]:
        ...
        try:
            ...
            print(f"[SNBSightDeposits] Error calculating next release: {e}")
            return None
        ...
        try:
            ...
            if next_release_str:
                ...
                if now > this_week_release and last_updated.astimezone(ZURICH) < this_week_release:
                    print(f"[SNBSightDeposits] Data is stale, this week's release was {this_week_release}")
                    return True

            # 7日以上経過していたら更新
            if (now - last_updated.astimezone(ZURICH)).days >= 7:
                print("[SNBSightDeposits] Cache is older than 7 days, refreshing")
                return True

            return False
        except Exception as e:
            print(f"[SNBSightDeposits] Error checking refresh: {e}")
            return True
    ...
    def _load_file_cache(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            print(f"[SNBSightDeposits] Failed to load file cache: {e}")
            return None
    ...
    def _save_file_cache(self, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            print(f"[SNBSightDeposits] Failed to save file cache: {e}")
...
# シングルトンインスタンス
snb_sight_deposits_service = SNBSightDepositsService()
