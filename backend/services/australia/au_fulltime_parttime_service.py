"""
Australian Bureau of Statistics (ABS) フルタイム/パートタイム雇用者数サービス

データソース:
- ABS SDMX API (Labour Force Survey)
- LF dataflow: M1 = Employed full-time, M2 = Employed part-time (thousands)
- Sex: Persons (3), Age: Total (1599), TSEST: Seasonally Adjusted (20)
- Region: Australia (AUS), Frequency: Monthly (M)

算出系列:
- fulltime: フルタイム雇用者数（千人）
- parttime: パートタイム雇用者数（千人）
- fulltime_mom: フルタイム前月増減（千人）
- parttime_mom: パートタイム前月増減（千人）
- fulltime_yoy: フルタイム前年比（%）
- parttime_yoy: パートタイム前年比（%）
...
DATA_CACHE_FILE = CACHE_DIR / "au_fulltime_parttime_cache.json"

# ABS API設定
ABS_API_BASE = "https://data.api.abs.gov.au/rest/data"

# LF: Labour Force Survey
# M1 = Employed full-time, M2 = Employed part-time (thousands)
LF_DATAFLOW = "ABS,LF,1.0.0"
LF_KEY = "M1+M2.3.1599.20.AUS.M"
LF_URL = f"{ABS_API_BASE}/{LF_DATAFLOW}/{LF_KEY}?startPeriod=2000-01&dimensionAtObservation=AllDimensions"

# FMPイベントパターン（失業率/雇用者数と同時発表）
FMP_EVENT_PATTERN = "Employment Change"


class AuFulltimeParttimeService:
    """ABS Full-time/Part-time Employment Service"""

    DATA_CACHE_KEY = "australia:au_fulltime_parttime:data"
    ...
    def _parse_sdmx_observations(
        ...
        """SDMX JSONレスポンスをパースして [{measure, time_period, value}, ...] を返す"""
        result = []
        try:
    ...
    def _build_data_points(
        ...
        2系列それぞれに value / mom / yoy を算出
        """
        # measure -> time_period -> value マップ
        ft_map: Dict[str, float] = {}  # Full-time
        pt_map: Dict[str, float] = {}  # Part-time
        ...
        # 全期間を取得
        all_periods = sorted(set(ft_map.keys()) | set(pt_map.keys()))
        ...
        for tp in all_periods:
            ...
            ft_val = round(ft_map[tp], 2) if tp in ft_map else None
            pt_val = round(pt_map[tp], 2) if tp in pt_map else None
            ...
            # Full-time MoM/YoY
            ft_mom = None
            if ft_val is not None and prev_tp in ft_map:
                ft_mom = round(ft_val - ft_map[prev_tp], 2)
            ft_yoy = None
            if ft_val is not None and prev_year_tp in ft_map and ft_map[prev_year_tp] > 0:
                ft_yoy = round(((ft_val - ft_map[prev_year_tp]) / ft_map[prev_year_tp]) * 100, 2)

            # Part-time MoM/YoY
            pt_mom = None
            if pt_val is not None and prev_tp in pt_map:
                pt_mom = round(pt_val - pt_map[prev_tp], 2)
            pt_yoy = None
            if pt_val is not None and prev_year_tp in pt_map and pt_map[prev_year_tp] > 0:
                pt_yoy = round(((pt_val - pt_map[prev_year_tp]) / pt_map[prev_year_tp]) * 100, 2)
        ...
        logger.info(f"Built {len(data_points)} full-time/part-time data points")
        return data_points
                ...
                if data_points:
                    ...
                    result = {
                        ...
                        "metadata": {
                            ...
                            "indicator": "Employed Persons Full-time/Part-time",
                            "frequency": "monthly",
                            "unit": "thousands",
                        },
    ...
    def get_cache_status(self) -> Dict[str, Any]:
        ...
        return {
            "indicator": "AU Full-time/Part-time Employment",
            "source": "ABS",
...
# シングルトンインスタンス
au_fulltime_parttime_service = AuFulltimeParttimeService()
