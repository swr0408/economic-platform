"""
Australian Bureau of Statistics (ABS) 賃金物価指数（WPI）サービス

データソース:
- ABS SDMX API (Wage Price Index)
- WPI dataflow: MEASURE 2 = QoQ (%), MEASURE 3 = YoY (%)
...
DATA_CACHE_FILE = CACHE_DIR / "au_wage_price_index_cache.json"

# ABS API設定
ABS_API_BASE = "https://data.api.abs.gov.au/rest/data"
WPI_DATAFLOW = "ABS,WPI,1.2.0"
WPI_KEY = "2+3.THRPEB.7.TOT.20.AUS.Q"
WPI_URL = f"{ABS_API_BASE}/{WPI_DATAFLOW}/{WPI_KEY}?startPeriod=1997-Q3&dimensionAtObservation=AllDimensions"

FMP_EVENT_PATTERNS = ["Wage Price Index QoQ", "Wage Price Index YoY"]


class AuWagePriceIndexService:
    """ABS Wage Price Index Service"""

    DATA_CACHE_KEY = "australia:au_wage_price_index:data"
    ...
    def _parse_sdmx_observations(self, raw_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        """SDMX JSONレスポンスをパースし、QoQ/YoYを時期別に整理"""
        result = []
        try:
    ...
    def _build_data_points(self, observations: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for obs in observations:
            ...
            if tp not in period_data:
                period_data[tp] = {}
            if measure == "2":
                period_data[tp]["qoq"] = round(obs["value"], 2)
            elif measure == "3":
        ...
        logger.info(f"Built {len(data_points)} WPI data points")
        return data_points

    def get_au_wage_price_index_data(self, force_refresh: bool = False) -> Dict[str, Any]:
        """賃金物価指数データを取得（キャッシュ付き）"""
        ...
        raw_data = self._fetch_sdmx(WPI_URL)
        if raw_data:
    ...
    def _get_next_release(self) -> Optional[Dict[str, str]]:
        try:
            ...
            return get_next_release_by_pattern("Wage Price Index", "AU")
        except Exception as e:
    ...
    def get_cache_status(self) -> Dict[str, Any]:
        ...
        return {
            "indicator": "AU Wage Price Index", "source": "ABS",
...
au_wage_price_index_service = AuWagePriceIndexService()
