"""
オーストラリア 個人消費（Consumer Spending）サービス

データソース:
- ABS SDMX API (MODELLERS_DB dataflow)
- AK90_CON = Households ; Final consumption expenditure (Chain volume measures)
...
DATA_CACHE_FILE = CACHE_DIR / "au_consumer_spending_cache.json"

# ABS API設定
ABS_API_BASE = "https://data.api.abs.gov.au/rest/data"
DATAFLOW = "ABS,MODELLERS_DB,1.0.0"
DATA_KEY = "AK90_CON.20.Q"
DATA_URL = f"{ABS_API_BASE}/{DATAFLOW}/{DATA_KEY}?startPeriod=2000-Q1&dimensionAtObservation=AllDimensions"


class AuConsumerSpendingService:
    """オーストラリア個人消費サービス"""

    DATA_CACHE_KEY = "australia:au_consumer_spending:data"
    FMP_EVENT_PATTERN = "GDP Final Consumption QoQ"
    FMP_COUNTRY = "AU"
    ...
    def _fetch_sdmx(self, url: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            logger.info(f"Fetching ABS MODELLERS_DB data from {url}")
            ...
            logger.error(f"Error fetching ABS MODELLERS_DB data: {e}")
            return None
    ...
    def _parse_sdmx_observations(self, raw_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Error parsing SDMX MODELLERS_DB data: {e}")
        return result
    ...
    def _build_data_points(self, observations: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        logger.info(f"Built {len(data_points)} Consumer Spending data points")
        return data_points

    def get_au_consumer_spending_data(self, force_refresh: bool = False) -> Dict[str, Any]:
        """個人消費データを取得（キャッシュ付き）"""
                ...
                if data_points:
                    ...
                    result = {
                        ...
                        "metadata": {
                            ...
                            "indicator": "Consumer Spending (Chain Volume Measures)",
                            "frequency": "quarterly",
                            "unit": "%",
                        },
    ...
    def _get_next_release(self) -> Optional[Dict[str, str]]:
        try:
            ...
            if result:
                # ラベルを日本語に変換（例: "GDP Final Consumption QoQ (Q4)" → "個人消費 (Q4)"）
                label = result.get("label", "")
                import re
                q_match = re.search(r"\(Q[1-4]\)", label)
                quarter = q_match.group(0) if q_match else ""
                result["label"] = f"個人消費 {quarter}".strip()
            return result
        except Exception as e:
    ...
    def get_cache_status(self) -> Dict[str, Any]:
        ...
        return {
            "indicator": "AU Consumer Spending", "source": "ABS (MODELLERS_DB)",
...
au_consumer_spending_service = AuConsumerSpendingService()
