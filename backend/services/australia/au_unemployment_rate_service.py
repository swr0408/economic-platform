"""
Australian Bureau of Statistics (ABS) 失業率サービス

データソース:
- ABS SDMX API (Labour Force Survey)
- LF dataflow: M13 = Unemployment rate (%)
...
DATA_CACHE_FILE = CACHE_DIR / "au_unemployment_rate_cache.json"
...
class AuUnemploymentRateService:
    """ABS Unemployment Rate Service"""

    DATA_CACHE_KEY = "australia:au_unemployment_rate:data"
    ...
    def _build_data_points(
        ...
        logger.info(f"Built {len(data_points)} unemployment rate data points")
        return data_points

    def get_au_unemployment_rate_data(self, force_refresh: bool = False) -> Dict[str, Any]:
        """失業率データを取得（キャッシュ付き）"""
        # Redisキャッシュ
                ...
                if data_points:
                    ...
                    if force_refresh and existing_cached_data:
                        ...
                        if existing_date and new_date and new_date <= existing_date:
                            logger.warning(
                                f"[AuUnemploymentRate] force_refresh requested but API returned "
    ...
    def get_cache_status(self) -> Dict[str, Any]:
        ...
        return {
            "indicator": "AU Unemployment Rate",
            "source": "ABS",
...
# シングルトンインスタンス
au_unemployment_rate_service = AuUnemploymentRateService()
