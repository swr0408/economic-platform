"""
FMP次回発表日取得ユーティリティ（オーストラリア向け）

indicator_event_mappingテーブルのfmp_event_patternsを使用して、
FMP APIから次回発表日を取得する共通関数を提供。

使用方法:
    from services.australia.fmp_next_release_utils import get_next_release_by_pattern, should_refresh_by_pattern

    # パターンを指定して次回発表日を取得
    next_release = get_next_release_by_pattern('RBA Interest Rate Decision', country='AU')
"""
...
def get_next_release_by_pattern(
    ...
    Args:
        event_pattern: イベント名のパターン（例: "RBA Interest Rate Decision"）
        use_cache: Redisキャッシュを使用するか
        country: 国コード（デフォルト: "AU"）
    ...
    try:
        ...
        for event in events:
            ...
            if dt_utc and dt_utc.date() >= today:
                ...
                candidates.append({
                    ...
                    "datetime_sydney": dt_sydney.isoformat(),
                    "time_sydney": dt_sydney.strftime("%H:%M"),
                    "label": event_name,
        ...
        print(f"[AU FMP Utils] Error fetching next release by pattern '{event_pattern}' for country '{country}': {e}")
        return None
...
def _get_next_release_from_db(
    ...
    try:
        ...
        print(f"[AU FMP Utils] Error fetching from DB: {e}")
        return None
...
def should_refresh_by_pattern(
    ...
    try:
        ...
        # 発表時刻ちょうどの再取得でソース(ABS SDMX等)未反映のまま last_updated=now を
        # 刻むと、下のスケジュール判定が「発表消化済み」と誤認し次回発表まで永久凍結する
        # （AU utils は従来 max-age を持たず、四半期国民勘定では最大3ヶ月凍結した）。
        # 一定時間で必ず True を返し自己回復させる。
        ...
        print(f"[AU FMP Utils] Error checking refresh by pattern '{event_pattern}' for country '{country}': {e}")
        return False


def _get_last_release_datetime(event_patterns, country: str = "AU"):
    ...
    try:
        ...
        with SessionLocal() as session:
            for pattern in event_patterns:
                ...
                row = session.execute(
                    query, {"countries": [country], "pattern": f"%{pattern}%"}
                ).fetchone()
                if row:
                    dt_utc = row[0]
                    if dt_utc.tzinfo is None:
        ...
        print(f"[AU FMP Utils] _get_last_release_datetime error: {e}")
        return None
...
def resolve_last_updated_after_fetch(
    ...
    背景: ダッシュボード更新はFMP発表時刻（=ABS公式発表 11:30 AEST）に発火するが、
    ABS SDMX APIへの新月反映は数分遅れることがある。発表時刻ちょうどの取得で旧月を
    キャッシュし `last_updated=now(≧発表時刻)` を刻むと、発表認識型の should_refresh が
    「発表消化済み」と判断し次回発表/TTLまで再取得しない（凍結）。
    ...
    が再取得を促し続け、ABS反映後の最初のポーリングで新月を取り込める。
    ...
    print(
        f"[AU FMP Utils] lag-guard: source not yet advanced past release "
...
def invalidate_next_release_cache(event_pattern: str, country: str = "AU") -> bool:
    """
    次回発表日キャッシュを無効化
