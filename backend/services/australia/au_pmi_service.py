"""
オーストラリア S&P Global (Judo Bank) PMI サービス
...
データソース:
- CSV履歴データ (backend/data/csv_import/AU S&P*.csv)
...
DATA_CACHE_FILE = CACHE_DIR / "au_pmi_cache.json"

# CSVデータ
CSV_DIR = Path(__file__).parent.parent.parent / "data" / "csv_import"
CSV_FILES = {
    "manufacturing": CSV_DIR / "AU S&P製造業PMI.csv",
    "services": CSV_DIR / "AU S&PサービスPMI.csv",
    "composite": CSV_DIR / "AU S&P総合PMI.csv",
}

# FMPパターン（country='AU'でフィルタリング）
# FMPでは "S&P Global" と "Judo Bank" の両方のイベント名が存在する
# 汎用名（"Manufacturing PMI"等）でも登録されることがあるためフォールバック追加
EVENT_PATTERNS = {
    "manufacturing": ["S&P Global Manufacturing PMI", "Judo Bank Manufacturing PMI", "Manufacturing PMI"],
    "services": ["S&P Global Services PMI", "Judo Bank Services PMI", "Services PMI"],
    "composite": ["S&P Global Composite PMI", "Judo Bank Composite PMI", "Composite PMI"],
}
...
class AuPmiService:
    """オーストラリア S&P Global PMI サービス（CSV + DB + FMP方式、3系列）"""

    DATA_CACHE_KEY = "australia:au_pmi:data"

    def __init__(self):
        pass

    def get_au_pmi_data(self, force_refresh: bool = False) -> Dict[str, Any]:
    ...
    def _load_series(self, pmi_type: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            print(f"[AuPmi] {pmi_type}: Loaded {len(result)} records (merged)")

        except Exception as e:
            print(f"[AuPmi] Error loading {pmi_type}: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _load_from_csv(self, pmi_type: str) -> List[Dict[str, Any]]:
        ...
        try:
            if not csv_file or not csv_file.exists():
                print(f"[AuPmi] CSV file not found for {pmi_type}: {csv_file}")
                return result
            ...
            print(f"[AuPmi] {pmi_type}: Loaded {len(result)} records from CSV")

        except Exception as e:
            print(f"[AuPmi] Error loading {pmi_type} from CSV: {e}")
    ...
    def _load_from_db(self, pmi_type: str) -> List[Dict[str, Any]]:
        """DBから履歴データを取得"""
        result = []
        patterns = DB_PATTERNS.get(pmi_type, [])
        if not patterns:
            return result
        ...
        try:
            with SessionLocal() as session:
                # 複数パターンでOR検索
                conditions = " OR ".join(
                    [f"LOWER(event) LIKE LOWER(:pattern{i})" for i in range(len(patterns))]
                )
                query = text(f"""
                    SELECT
                        datetime_utc,
            ...
            print(f"[AuPmi] {pmi_type}: Loaded {len(result)} records from DB")

        except Exception as e:
            print(f"[AuPmi] Error loading {pmi_type} from DB: {e}")
    ...
    def _fetch_from_fmp(self, pmi_type: str) -> List[Dict[str, Any]]:
        """FMPから最新データを取得"""
        result = []
        patterns = EVENT_PATTERNS.get(pmi_type, [])
        if not patterns:
            return result
        ...
        try:
            ...
            for event in events:
                ...
                matched = any(p.lower() in event_name.lower() for p in patterns)
                if not matched:
                    continue
            ...
            print(f"[AuPmi] {pmi_type}: Fetched {len(result)} records from FMP")

        except Exception as e:
            print(f"[AuPmi] Error fetching {pmi_type} from FMP: {e}")
    ...
    def _save_to_db(self, event: dict) -> None:
        ...
        try:
            ...
            print(f"[AuPmi] Error saving to DB: {e}")

    def _get_next_release(self) -> Optional[Dict[str, Any]]:
        """次回発表日を取得（製造業PMIのスケジュールで代表）"""
        try:
            from services.australia.fmp_next_release_utils import get_next_release_by_pattern
            return get_next_release_by_pattern(EVENT_PATTERNS["manufacturing"][0], country=COUNTRY)
        except Exception as e:
            print(f"[AuPmi] Error getting next release: {e}")
            return None
    ...
    def _should_refresh(self, last_updated_str: str) -> bool:
        ...
        try:
            ...
            return should_refresh_by_pattern(EVENT_PATTERNS["manufacturing"][0], last_updated_str, country=COUNTRY)
        except Exception:
            try:
    ...
    def _load_file_cache(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            print(f"[AuPmi] Failed to load file cache: {e}")
            return None
    ...
    def _save_file_cache(self, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            print(f"[AuPmi] Failed to save file cache: {e}")
    ...
    def get_cache_status(self) -> Dict[str, Any]:
        ...
        return {
            "indicator": "AU S&P Global PMI (Manufacturing/Services/Composite)",
