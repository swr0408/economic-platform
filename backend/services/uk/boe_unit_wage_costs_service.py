"""
BOE Unit Wage Costs Service
BOE Projections DatabankからUnit Wage Costs（単位賃金コスト）データを取得

データソース: Projections Databank - {Month} {Year} MPR.xlsx
- Sheet: 31. Unit wage costs
...
class BOEUnitWageCostsService(BOEServiceBase):
    """BOE Unit Wage Costs サービス"""

    DATA_CACHE_KEY = "uk:boe_unit_wage_costs:data"
    CACHE_FILE_PATH = CACHE_DIR / "boe_unit_wage_costs_cache.json"
    DATA_KEY = "unit_wage_costs"
    SHEET_NAME = "31. Unit wage costs"

    def _fetch_data_from_source(self) -> Optional[Dict[str, Any]]:
        """MPRからUnit Wage Costsデータを取得"""
        try:
            mpr_info = get_mpr_info()
            latest_info = mpr_info['latest']
            previous_info = mpr_info['previous']

            logger.info(f"Fetching UWC for {latest_info['month_name'].title()} {latest_info['year']}")
            ...
            if latest_data is None or latest_data.get('latest') is None:
                logger.warning("UWC not found in latest MPR, trying previous")
            ...
            logger.error(f"Error fetching BOE UWC: {e}")
            return None

    def _extract_data_from_sheet(self, ws) -> Dict[str, Any]:
        """シートからUnit Wage Costsデータを抽出"""
        ...
        logger.info(f"Extracted UWC data: latest={latest_date}")
        return data


# シングルトンインスタンス
boe_unit_wage_costs_service = BOEUnitWageCostsService()
