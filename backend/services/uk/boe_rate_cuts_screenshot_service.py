"""
BOE Rate Cuts Expectation Screenshot Service
Captures screenshots from MacroMicro charts via PlaywrightRunner

URLs (2026年):
- 年末金利予想: https://en.macromicro.me/series/78280/uk-boe-year-end-interest-rate-expectation-2026
- 利下げ回数: https://en.macromicro.me/series/78286/uk-boe-interest-rate-cuts-expectation-2026
...
CACHE_DIR = Path(__file__).parent.parent.parent / "data" / "cache" / "uk" / "policy"
CACHE_DIR.mkdir(parents=True, exist_ok=True)

YEAREND_SCREENSHOT_FILENAME = "boe_yearend_rate_expectations.png"
RATE_CUTS_SCREENSHOT_FILENAME = "boe_rate_cuts_expectations.png"
...
YEAREND_URL = "https://en.macromicro.me/series/78280/uk-boe-year-end-interest-rate-expectation-2026"
RATE_CUTS_URL = "https://en.macromicro.me/series/78286/uk-boe-interest-rate-cuts-expectation-2026"


class BOERateCutsScreenshotService:
    """BOE Rate Cuts Expectation Screenshot Service"""

    CACHE_KEY = "uk:boe_rate_cuts_screenshot:metadata"
    ...
    def capture_all_screenshots(self, force_refresh: bool = False) -> Dict[str, Any]:
        ...
        for key, url, path, filename in items:
            if not force_refresh and path.exists():
                ...
                if file_age < 86400:
                    results[key] = {
                        "success": True,
                        "url": f"/cache/uk/policy/{filename}",
                        "cached": True,
                    }
                    logger.info(
                        f"[BOERateCuts] Using cached {key} screenshot "
        ...
        for idx, (key, url, path, filename) in enumerate(to_capture):
            ...
            try:
                ...
                logger.error(f"[BOERateCuts] {key} capture failed: {e}")
                results[key] = {
                    "success": False,
                    "url": None,
                    "cached": False,
    ...
    def get_screenshot_urls(self) -> Dict[str, Any]:
        ...
        if YEAREND_SCREENSHOT_PATH.exists():
            result["yearend_url"] = f"/cache/uk/policy/{YEAREND_SCREENSHOT_FILENAME}"

        if RATE_CUTS_SCREENSHOT_PATH.exists():
            result["rate_cuts_url"] = f"/cache/uk/policy/{RATE_CUTS_SCREENSHOT_FILENAME}"
    ...
    def get_cache_status(self) -> Dict[str, Any]:
        cached_meta = redis_client.get(self.CACHE_KEY)
        return {
            "indicator": "BOE Rate Cuts Expectations Screenshots",
            "source": "MacroMicro",
...
boe_rate_cuts_screenshot_service = BOERateCutsScreenshotService()
