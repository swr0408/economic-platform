"""
BOE GDP Forecast Service
BOE MPRデータからGDP成長見通しデータを取得

データソース (November 2025以降):
- Projections Databank - {Month} {Year} MPR.xlsx
  - 2. GDP growth シート (見通し)
  - 34. GDP growth distribution シート (ファン/分布) - 任意
  - 3. GDP level シート (水準) - 参考

データソース (旧構造):
- Parameters for MPC GDP growth projections based on market interest rate expectations.xlsx
...
DATA_CACHE_FILE = CACHE_DIR / "boe_gdp_forecast_cache.json"


class BOEGDPForecastService:
    """BOE GDP Forecast サービス"""

    DATA_CACHE_KEY = "uk:boe_gdp_forecast:data"
    SHEET_NAME = "2. GDP growth"
    DISTRIBUTION_SHEET = "34. GDP growth distribution"
    LEVEL_SHEET = "3. GDP level"
    ...
    def fetch_data(self, force_refresh: bool = False) -> Dict[str, Any]:
        ...
        # MPRからデータを取得
        data = self._fetch_gdp_forecast_data()
    ...
    def _parse_gdp_from_databank(self, wb) -> Optional[Dict]:
        """Projections DatabankからGDP growthデータを抽出

        2. GDP growth シート構造:
        - Row 5: Column headers (quarters: 2007 Q3, 2007 Q4, ...)
        ...
        try:
            ...
            if not quarters:
                logger.error("No quarters found in GDP growth sheet")
                return None
            ...
            logger.error(f"Error parsing GDP from Databank: {e}")
            return None

    def _parse_old_gdp_projections(self, wb) -> Optional[Dict]:
        """旧構造をパース"""
        try:
            # Find sheet with GDP projections
            sheet_name = None
            for name in wb.sheetnames:
                if 'GDP' in name.upper() and 'Forecast' in name:
                    sheet_name = name
                    break

            if not sheet_name:
                for name in wb.sheetnames:
                    if 'GDP' in name.upper():
                        sheet_name = name
                        break

            if not sheet_name:
                logger.error("No GDP sheet found in workbook")
                return None
            ...
            logger.error(f"Error parsing old GDP projections: {e}")
            return None

    def _fetch_gdp_forecast_data(self) -> Optional[Dict]:
        """最新MPRからGDP見通しデータを取得"""
        try:
            ...
            logger.info(f"Fetching GDP forecast for {latest_info['month_name'].title()} {latest_info['year']}")
            ...
            if result:
                wb, _ = result
                parsed_data = self._parse_gdp_from_databank(wb)
            ...
            logger.info("Trying old structure for GDP projections")
            ...
            if zip_content:
                ...
                gdp_file = None
                for f in files:
                    if 'GDP growth projections' in f and f.endswith('.xlsx'):
                        gdp_file = f
                        break

                if gdp_file:
                    wb = extract_workbook_from_zip(zip_content, gdp_file)
                    if wb:
                        parsed_data = self._parse_old_gdp_projections(wb)
                        if parsed_data:
                            return {
                                "table_data": parsed_data.get("table_data", []),
                                "chart_data": None,
                                "metadata": {
                                    ...
                                    "last_updated": datetime.now().isoformat(),
                                    "source": "Bank of England",
                                    "data_source": "Parameters for MPC GDP..."
                                }
                            }

            # Try previous MPR
            logger.warning(f"GDP data not found in {latest_info['month_name'].title()} MPR, trying previous")
            ...
            logger.error("Failed to fetch GDP forecast from both MPRs")
            return None
    ...
    def _get_fallback_data(self) -> Dict:
        ...
        return {
            ...
            "metadata": {
                "last_updated": datetime.now().isoformat(),
                "source": "Bank of England",
                "error": "Failed to fetch BOE GDP forecast data"
            },
            "cached": False,
            "source": "none",
        }
...
# シングルトンインスタンス
boe_gdp_forecast_service = BOEGDPForecastService()
