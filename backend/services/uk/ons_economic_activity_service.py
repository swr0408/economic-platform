"""
UK ONS経済活動率サービス
ONS Time Series APIから経済活動率データを取得

指標:
- LF22: Economic Activity Rate (aged 16-64, seasonally adjusted)
...
発表スケジュール:
- FMPカレンダーから自動取得（Unemployment Rateパターン）
...
DATA_CACHE_FILE = CACHE_DIR / "ons_economic_activity_cache.json"


class ONSEconomicActivityService:
    """UK ONS経済活動率サービス"""

    DATA_CACHE_KEY = "uk:ons_economic_activity:data"
    ECONALPHA_ID = "ons_economic_activity"

    # ONS Time Series API URL
    # LF22: Economic Activity Rate (aged 16-64, seasonally adjusted)
    ECONOMIC_ACTIVITY_CSV_URL = "https://www.ons.gov.uk/generator?format=csv&uri=/employmentandlabourmarket/peopleinwork/employmentandemployeetypes/timeseries/lf22/lms"

    # FMPカレンダー検索パターン（失業率と同じスケジュール）
    FMP_EVENT_PATTERN = "Unemployment Rate"

    def __init__(self):
        pass

    def get_ons_economic_activity_data(self, force_refresh: bool = False) -> Dict[str, Any]:
        """ONS経済活動率データを取得"""
        # 次回発表日を取得
    ...
    def _get_next_release(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            print(f"[ONS Economic Activity] Error getting next release: {e}")
            return None

    def _fetch_from_ons(self) -> Optional[Dict[str, Any]]:
        """ONSからCSVデータを取得してパース"""
        try:
            print(f"[ONS Economic Activity] Fetching data from {self.ECONOMIC_ACTIVITY_CSV_URL}")

            headers = {
                "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"
            }
            response = requests.get(self.ECONOMIC_ACTIVITY_CSV_URL, headers=headers, timeout=30)
            response.raise_for_status()

            print(f"[ONS Economic Activity] Received {len(response.text)} bytes")
            ...
            title = title_row[1] if len(title_row) > 1 else "Economic Activity Rate"
            cdid = cdid_row[1] if len(cdid_row) > 1 else "LF22"
                        ...
                        if " " in period_str and len(period_str.split()) == 2:
                            ...
                            if len(parts[0]) == 4 and parts[0].isdigit():
                                ...
                                if month_str in month_map:
                                    ...
                                    # ONS経済活動率(LF22/LFS)も3ヶ月移動平均で、ONSは値を
                                    # 期間の「真ん中の月」で日付付けする(例: 2026 MAR = Feb-Apr期)。
                                    # 失業率/雇用者数チャートと整合させ最終月ラベルで表示する
                                    # ため +1ヶ月して期間の最終月に揃える。
            ...
            print(f"[ONS Economic Activity] Extracted {len(data_points)} data points")
            ...
            return {
                ...
                "metadata": {
                    ...
                    "description": "イギリス経済活動率（16-64歳、季節調整済）",
                },
            }

        except Exception as e:
            print(f"[ONS Economic Activity] Error fetching from ONS: {e}")
            import traceback
            traceback.print_exc()
            return None
        ...
        try:
            ...
            try:
                if should_refresh_by_pattern(self.FMP_EVENT_PATTERN, last_updated_str, country="GB"):
                    print(f"[ONS Economic Activity] FMP pattern indicates refresh needed")
                    return True
            except Exception as e:
                print(f"[ONS Economic Activity] Error checking FMP refresh: {e}")

            return False

        except Exception as e:
            print(f"[ONS Economic Activity] Error in should_refresh: {e}")
            return True
    ...
    def _load_file_cache(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            print(f"[ONS Economic Activity] Failed to load file cache: {e}")
            return None
    ...
    def _save_file_cache(self, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            print(f"[ONS Economic Activity] Failed to save file cache: {e}")
    ...
    def get_cache_status(self) -> Dict[str, Any]:
        ...
        return {
            "indicator": "ONS Economic Activity",
            "source": "Office for National Statistics",
...
# シングルトンインスタンス
ons_economic_activity_service = ONSEconomicActivityService()
