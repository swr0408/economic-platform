"""
UK ONS失業給付申請件数サービス
ONS Time Series APIから失業給付申請件数データを取得

指標:
- BCJD: Claimant Count (UK, seasonally adjusted, thousands)
...
DATA_CACHE_FILE = CACHE_DIR / "ons_claimant_count_cache.json"


class ONSClaimantCountService:
    """UK ONS失業給付申請件数サービス"""

    DATA_CACHE_KEY = "uk:ons_claimant_count:data"
    ECONALPHA_ID = "ons_claimant_count"

    # ONS Time Series API - BCJD series (Claimant Count, seasonally adjusted, thousands)
    BCJD_CSV_URL = "https://www.ons.gov.uk/generator?format=csv&uri=/employmentandlabourmarket/peoplenotinwork/outofworkbenefits/timeseries/bcjd/unem"

    # FMPカレンダー検索パターン
    FMP_EVENT_PATTERNS = ["Claimant Count Change"]

    def __init__(self):
        pass

    def get_ons_claimant_count_data(self, force_refresh: bool = False) -> Dict[str, Any]:
        """ONS失業給付申請件数データを取得"""
        # 次回発表日を取得
    ...
    def _get_next_release(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            print(f"[ONS Claimant Count] Error getting next release: {e}")
            return None

    def _fetch_from_ons(self) -> Optional[Dict[str, Any]]:
        """ONSからCSVデータを取得してパース"""
        try:
            print(f"[ONS Claimant Count] Fetching data from {self.BCJD_CSV_URL}")

            headers = {
                "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"
            }
            response = requests.get(self.BCJD_CSV_URL, headers=headers, timeout=30)
            response.raise_for_status()

            print(f"[ONS Claimant Count] Received {len(response.text)} bytes")

            return self._parse_csv_data(response.text)

        except requests.exceptions.RequestException as e:
            print(f"[ONS Claimant Count] Request error: {e}")
            return None
        except Exception as e:
            print(f"[ONS Claimant Count] Error fetching from ONS: {e}")
            import traceback
            traceback.print_exc()
            return None
    ...
    def _parse_csv_data(self, csv_text: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            title = title_row[1] if len(title_row) > 1 else "Claimant Count"
            cdid = cdid_row[1] if len(cdid_row) > 1 else "BCJD"
            unit = unit_row[1] if len(unit_row) > 1 else "Thousands"
            ...
            # 前月比（MoM変化）と前年比（YoY変化）を計算
            for i, point in enumerate(monthly_data_points):
                # 前月比
                if i > 0:
                    prev_value = monthly_data_points[i - 1]["value"]
                    if prev_value is not None:
                        point["mom"] = round(point["value"] - prev_value, 1)
                else:
                    point["mom"] = None

                # 前年比（12ヶ月前）
                if i >= 12:
                    prev_year_value = monthly_data_points[i - 12]["value"]
                    if prev_year_value is not None and prev_year_value != 0:
                        yoy_change = ((point["value"] - prev_year_value) / prev_year_value) * 100
                        point["yoy"] = round(yoy_change, 2)
                    else:
                        point["yoy"] = None
                else:
                    point["yoy"] = None

            print(f"[ONS Claimant Count] Extracted {len(monthly_data_points)} monthly data points")
            ...
            return {
                ...
                "metadata": {
                    ...
                    "description": "イギリス失業給付申請件数（季節調整済み、千人）",
                },
            }

        except Exception as e:
            print(f"[ONS Claimant Count] Error parsing CSV: {e}")
            import traceback
            traceback.print_exc()
            return None
            ...
            try:
                ...
                for pattern in self.FMP_EVENT_PATTERNS:
                    if should_refresh_by_pattern(pattern, last_updated_str, country="GB"):
                        print(f"[ONS Claimant Count] FMP pattern '{pattern}' indicates refresh needed")
                        return True
            except Exception as e:
                print(f"[ONS Claimant Count] Error checking FMP refresh: {e}")

            return False

        except Exception as e:
            print(f"[ONS Claimant Count] Error in should_refresh: {e}")
            return True
    ...
    def _load_file_cache(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            print(f"[ONS Claimant Count] Failed to load file cache: {e}")
            return None
    ...
    def _save_file_cache(self, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            print(f"[ONS Claimant Count] Failed to save file cache: {e}")
...
# シングルトンインスタンス
ons_claimant_count_service = ONSClaimantCountService()
