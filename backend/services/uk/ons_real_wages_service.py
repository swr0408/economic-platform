"""
UK ONS実質平均週給サービス
ONS Time Series APIから実質賃金伸び率データを取得

指標:
- A2FA: Real Regular Pay (excluding bonuses) YoY growth %
- A3WW: Real Total Pay (including bonuses) YoY growth %
...
DATA_CACHE_FILE = CACHE_DIR / "ons_real_wages_cache.json"


class ONSRealWagesService:
    """UK ONS実質平均週給サービス"""

    DATA_CACHE_KEY = "uk:ons_real_wages:data"
    ECONALPHA_ID = "ons_wages"  # 同じFMPイベントを使用

    # ONS Time Series API URLs
    # A2FA: Real Regular Pay (excluding bonuses) YoY growth %
    REGULAR_PAY_CSV_URL = "https://www.ons.gov.uk/generator?format=csv&uri=/employmentandlabourmarket/peopleinwork/earningsandworkinghours/timeseries/a2fa/emp"
    # A3WW: Real Total Pay (including bonuses) YoY growth %
    TOTAL_PAY_CSV_URL = "https://www.ons.gov.uk/generator?format=csv&uri=/employmentandlabourmarket/peopleinwork/earningsandworkinghours/timeseries/a3ww/emp"

    # FMPカレンダー検索パターン（名目賃金と同じ発表日）
    FMP_EVENT_PATTERNS = [
    ...
    def get_ons_real_wages_data(self, force_refresh: bool = False) -> Dict[str, Any]:
        """ONS実質平均週給データを取得"""
        # 次回発表日を取得
    ...
    def _get_next_release(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            print(f"[ONS Real Wages] Error getting next release: {e}")
            return None

    def _fetch_from_ons(self) -> Optional[Dict[str, Any]]:
        """ONSから両系列のCSVデータを取得してパース"""
        try:
            print(f"[ONS Real Wages] Fetching Real Regular Pay data from {self.REGULAR_PAY_CSV_URL}")
            regular_pay_data = self._parse_csv_data(self.REGULAR_PAY_CSV_URL, "Real Regular Pay (A2FA)")
            if not regular_pay_data:
                return None

            print(f"[ONS Real Wages] Fetching Real Total Pay data from {self.TOTAL_PAY_CSV_URL}")
            total_pay_data = self._parse_csv_data(self.TOTAL_PAY_CSV_URL, "Real Total Pay (A3WW)")
            if not total_pay_data:
                return None

            # 両系列を統合
            return self._combine_series(regular_pay_data, total_pay_data)

        except Exception as e:
            print(f"[ONS Real Wages] Error fetching from ONS: {e}")
            import traceback
            traceback.print_exc()
            return None
    ...
    def _parse_csv_data(self, csv_url: str, series_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            print(f"[ONS Real Wages] Received {len(response.text)} bytes for {series_name}")
            ...
            print(f"[ONS Real Wages] Extracted {len(data_points)} data points for {series_name}")
            ...
            print(f"[ONS Real Wages] Error parsing CSV for {series_name}: {e}")
            import traceback
            traceback.print_exc()
            return None

    def _combine_series(
        self, regular_pay_data: Dict[str, Any], total_pay_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """両系列を統合"""
        combined_data = {}
        ...
        for point in total_pay_data["data"]:
            ...
            if date in combined_data:
                combined_data[date]["total_pay"] = point["value"]
            else:
                combined_data[date] = {
                    "date": date,
                    "regular_pay": None,
        ...
        print(f"[ONS Real Wages] Combined {len(sorted_data)} data points")
        ...
        return {
            ...
            "metadata": {
                ...
                "unit": "%",
                "release_date": regular_pay_data["metadata"]["release_date"],
                "next_release_ons": regular_pay_data["metadata"]["next_release_ons"],
                "source": "Office for National Statistics",
                "description": "イギリス実質平均週給（前年比）",
            },
        }
            ...
            try:
                ...
                for pattern in self.FMP_EVENT_PATTERNS:
                    if should_refresh_by_pattern(pattern, last_updated_str, country="GB"):
                        print(f"[ONS Real Wages] FMP pattern '{pattern}' indicates refresh needed")
                        return True
            except Exception as e:
                print(f"[ONS Real Wages] Error checking FMP refresh: {e}")

            return False

        except Exception as e:
            print(f"[ONS Real Wages] Error in should_refresh: {e}")
            return True
    ...
    def _load_file_cache(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            print(f"[ONS Real Wages] Failed to load file cache: {e}")
            return None
    ...
    def _save_file_cache(self, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            print(f"[ONS Real Wages] Failed to save file cache: {e}")
...
# シングルトンインスタンス
ons_real_wages_service = ONSRealWagesService()
