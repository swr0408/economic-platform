"""
ECB SPF (Survey of Professional Forecasters) サービス
ECBからインフレ期待データを取得

指標:
- HICP 1年先予測 (One year ahead)
- HICP 2年先予測 (Two years ahead)
- HICP 長期予測 (Longer term - five years ahead)
...
DATA_CACHE_FILE = CACHE_DIR / "ecb_spf_cache.json"


class ECBSPFService:
    """ECB SPFサービス"""

    DATA_CACHE_KEY = "eurozone:ecb_spf:data"
    ...
    SERIES_KEYS = {
        "hicp_12m": "M.U2.HICP.POINT.P12M.Q.AVG",     # 12ヶ月先
        "hicp_24m": "M.U2.HICP.POINT.P24M.Q.AVG",     # 24ヶ月先
        "hicp_lt": "Q.U2.HICP.POINT.LT.Q.AVG",        # 長期（5年先）
    }
    ...
    def get_ecb_spf_data(self, force_refresh: bool = False) -> Dict[str, Any]:
        """SPFデータを取得"""
        # Redisキャッシュチェック
        ...
        if api_result:
            ...
            cache_payload = {
                ...
                "metadata": {
                    "source": "European Central Bank (ECB) - Survey of Professional Forecasters",
                    "indicator": "SPF - Inflation Expectations",
                    "data_start": "2015-01-01",
                    "description": "インフレ期待（ユーロ圏・SPF）",
                },
    ...
    def _fetch_from_ecb(self, start_date: str = "2015-01-01") -> Optional[Dict[str, Any]]:
        """ECB APIからSPFデータを取得"""
        try:
            print("[ECBSPF] Fetching SPF data from ECB API")
            ...
            for key, series_key in self.SERIES_KEYS.items():
                ...
                if data:
                    ...
                    print(f"[ECBSPF] {key}: {len(data)} data points")

            if not inflation_expectations:
                print("[ECBSPF] No data fetched from ECB API")
                return None

            return {"inflation_expectations": inflation_expectations}

        except Exception as e:
            print(f"[ECBSPF] Error fetching from ECB: {e}")
            import traceback
            traceback.print_exc()
            return None
    ...
    def _fetch_series_data(self, series_key: str, start_date: str, data_key: str) -> Optional[List[Dict]]:
        ...
        - hicp_12m: 予測対象期 - 12ヶ月 = 発表期
        - hicp_24m: 予測対象期 - 24ヶ月 = 発表期
        - hicp_lt: そのまま四半期形式（発表期）
        """
        ...
        try:
            ...
            if "dataSets" not in data or len(data["dataSets"]) == 0:
                print(f"[ECBSPF] No datasets for {series_key}")
                return None

            dataset = data["dataSets"][0]
            if "series" not in dataset:
                print(f"[ECBSPF] No series in dataset for {series_key}")
                return None
            ...
            if not time_dimension:
                print(f"[ECBSPF] No time dimension for {series_key}")
                return None
            ...
            print(f"[ECBSPF] Request error for {series_key}: {e}")
            return None
        except Exception as e:
            print(f"[ECBSPF] Parse error for {series_key}: {e}")
            return None
    ...
    def _convert_target_to_survey_date(self, target_date: str, data_key: str) -> str:
        ...
        Args:
            ...
            data_key: データキー（"hicp_12m", "hicp_24m", "hicp_lt"）
        ...
        try:
            ...
            if data_key == "hicp_12m":
                # 1年先予測: 予測対象期 - 9ヶ月 = 発表期
                # 例: 2026-09 → 2025-12 → 2025-Q4
                months_back = 9
            elif data_key == "hicp_24m":
            ...
            print(f"[ECBSPF] Error converting date {target_date}: {e}")
            return target_date
        ...
        try:
            ...
            if match:
                ...
                try:
                    ...
                    print(f"[ECBSPF] Could not parse date: {date_str}")

            print("[ECBSPF] Could not find next release date on ECB website")
            return self._calculate_next_release()

        except Exception as e:
            print(f"[ECBSPF] Error fetching next release from ECB: {e}")
            return self._calculate_next_release()
    ...
    def _calculate_next_release(self) -> Optional[str]:
        ...
        try:
            ...
            print(f"[ECBSPF] Error calculating next release: {e}")
            return None
    ...
    def _should_refresh(self, last_updated_str: str) -> bool:
        ...
        try:
                    ...
                    if now > next_release:
                        ...
                        if days_since_release <= 14:
                            ...
                            if hours_since_update >= 24:
                                print(f"[ECBSPF] Past release date ({days_since_release} days), checking for new data and next release")
                                return True
                        else:
                            # 2週間経過後も次回発表日が古いままなら2週間ごとに再取得
                            if hours_since_update >= 24 * 14:
                                print("[ECBSPF] Next release date is stale, refreshing")
                                return True
                        return False
            ...
            if now.month in [1, 4, 7, 10]:
                if hours_since_update >= 24:
                    print("[ECBSPF] Release month, daily check (no next_release set)")
                    return True
            else:
                # 発表月以外でも2週間ごとに次回発表日を再取得
                if hours_since_update >= 24 * 14:
                    print("[ECBSPF] Periodic check for next release date")
                    return True

            return False

        except Exception as e:
            print(f"[ECBSPF] Error in should_refresh: {e}")
            return True
    ...
    def _load_file_cache(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            print(f"[ECBSPF] Failed to load file cache: {e}")
            return None
    ...
    def _save_file_cache(self, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            print(f"[ECBSPF] Failed to save file cache: {e}")
    ...
    def get_cache_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "data_count": {
                "hicp_12m": len(inflation_expectations.get("hicp_12m", [])),
                "hicp_24m": len(inflation_expectations.get("hicp_24m", [])),
                "hicp_lt": len(inflation_expectations.get("hicp_lt", [])),
            } if cached_data else {},
