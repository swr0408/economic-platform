...
def should_refresh_by_fmp_schedule(
    ...
    try:
        ...
        with SessionLocal() as session:
            ...
            #    FMPがactualを埋めていなくても、データソースには既にデータがある可能性
...
def should_refresh_by_pattern(
    ...
    Args:
        ...
        max_age_hours: max-ageフォールバックの上限（時間）。これを超えて未更新なら
            発表日判定に関係なく強制リフレッシュ。デフォルト168h(7日)。
            源泉が発表当日〜翌日に反映される指標は24〜36hを指定して回復を早める。
            FMP発表時刻ちょうどに更新が走り last_updated が発表時刻より後になると
            `last_updated < release` が永久に不成立になる凍結を、この net で回復させる。
    ...
    try:
        ...
        # max-age フォールバック（自己回復ネット）:
        # FMPイベント欠落・発表時刻レースで last_updated が発表時刻より後になり
        # 発表日判定が永久に False を返し続ける凍結を防ぐ。
        if (now - last_updated).total_seconds() > max_age_hours * 60 * 60:
            return True
...
def _get_last_release_datetime(event_patterns, country: str = "EU") -> Optional[datetime]:
    ...
    try:
        ...
        print(f"[EU FMP Utils] _get_last_release_datetime error: {e}")
        return None
...
def resolve_last_updated_after_fetch(
    ...
    背景: ダッシュボード更新はFMP発表時刻（=DG ECFIN フラッシュ発表 16:00 CET）に発火するが、
    チャートのソース Eurostat `ei_bsco_m` へのフラッシュ反映は数時間〜数日遅れる。発表時刻
    直後の取得で旧月をキャッシュし `last_updated=now(≧発表時刻)` を刻むと
    should_refresh_by_pattern が「発表消化済み」と判断し次回発表（翌月）まで再取得しない（凍結）。
    ...
    が再取得を促し続け、Eurostat反映（または FMP速報補完）後の最初のポーリングで新月を取り込める。
    ...
    print(
        f"[EU FMP Utils] lag-guard: source not yet advanced past release "
