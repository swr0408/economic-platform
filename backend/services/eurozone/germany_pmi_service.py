...
DATA_CACHE_FILE = CACHE_DIR / "germany_pmi_cache.json"


class GermanyPMIService:
    """ドイツ S&P Global PMIサービス"""

    DATA_CACHE_KEY = "eurozone:economy:germany_pmi:data"
    ECONALPHA_ID = "germany_pmi"

    # DBクエリ用のイベントパターン（国コードでフィルタリング）
    EVENT_PATTERNS = {
        "manufacturing": ["Manufacturing PMI", "S&P Global Manufacturing PMI"],
        "services": ["Services PMI", "S&P Global Services PMI"],
        "composite": ["Composite PMI", "S&P Global Composite PMI"],
    }

    def __init__(self):
        pass

    def get_germany_pmi_data(
        self,
    ...
    def _load_from_db(self, pmi_type: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            with SessionLocal() as session:
                ...
                print(f"[GermanyPMI] Loaded {len(result)} DE {pmi_type.capitalize()} PMI records from DB")
                return result

        except Exception as e:
            print(f"[GermanyPMI] Error loading DE {pmi_type} PMI from DB: {e}")
            import traceback
            traceback.print_exc()
            return []
    ...
    def _load_file_cache(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            print(f"[GermanyPMI] Failed to load file cache: {e}")
            return None
    ...
    def _save_file_cache(self, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            print(f"[GermanyPMI] Cache saved to {DATA_CACHE_FILE}")
        except Exception as e:
            print(f"[GermanyPMI] Failed to save file cache: {e}")
    ...
    def get_cache_status(self) -> Dict[str, Any]:
        ...
        return {
            "indicator": "Germany S&P Global PMI (Manufacturing/Services/Composite)",
            "source": "Database (FMP)",
...
# シングルトンインスタンス
germany_pmi_service = GermanyPMIService()
