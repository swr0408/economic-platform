...
注意事項:
- 月に2度発表（速報値・確報値）
- country='EU'でフィルタリング（他国のPMIを除外）
- HCOBは2023年以降の名称、それ以前はS&P Global/Markit
"""
import json
...
class EurozonePMIService:
    """ユーロ圏 HCOB PMIサービス"""

    DATA_CACHE_KEY = "eurozone:economy:eu_pmi:data"

    # 3つのPMI指標に対応するECONALPHA_ID
    ECONALPHA_IDS = {
        "manufacturing": "ea_hcob_manufacturing_pmi",
        "services": "ea_hcob_services_pmi",
        "composite": "ea_hcob_composite_pmi",
    }
        ...
        try:
            ...
            with SessionLocal() as session:
                ...
                for row in rows:
                    ...
                    if dt_utc:
                        # イベント名から対象月を抽出（例: "HCOB Manufacturing PMI (Dec)"）
                ...
                print(f"Loaded {len(result)} EU HCOB {pmi_type.capitalize()} PMI records from DB")
                return result

        except Exception as e:
            print(f"Error loading EU HCOB {pmi_type} PMI from DB: {e}")
            import traceback
            traceback.print_exc()
            return []
    ...
    def get_cache_status(self) -> Dict[str, Any]:
        ...
        return {
            "indicator": "EU HCOB PMI (Manufacturing/Services/Composite)",
            "source": "Database (FMP)",
...
# シングルトンインスタンス
eu_pmi_service = EurozonePMIService()
