...
DATA_CACHE_FILE = CACHE_DIR / "indeed_euro_wage_cache.json"
...
class IndeedEuroWageService:
    """Indeed賃金トラッカー（ユーロ圏）サービス"""

    DATA_CACHE_KEY = "eurozone:indeed_euro_wage:data"
    LAST_CHECK_KEY = "eurozone:indeed_euro_wage:last_check"
    MONTHLY_UPDATE_KEY = "eurozone:indeed_euro_wage:monthly_updated"
    ...
    def get_data(self, force_refresh: bool = False) -> Dict[str, Any]:
        ...
        if csv_data is not None:
            germany_data = self._process_country_data(csv_data, "DE")
            france_data = self._process_country_data(csv_data, "FR")
            euro_area_data = self._process_country_data(csv_data, "EA")

            # 前回のデータと比較して更新されているかチェック
            cached_data = redis_client.get(self.DATA_CACHE_KEY)
            is_new_data = self._is_data_updated(cached_data, euro_area_data)

            if is_new_data:
                # 新しいデータがあった場合、今月の更新フラグを立てる
                self._set_monthly_updated()
                print(f"[IndeedEuroWage] New data detected, setting monthly update flag")

            # 最新値を取得（ユーロ圏を優先）
            latest = euro_area_data[-1] if euro_area_data else None
    ...
    def _fetch_csv_data(self) -> Optional[pd.DataFrame]:
        ...
        """
        try:
            print(f"[IndeedEuroWage] Fetching from GitHub: {INDEED_WAGE_URL}")
            headers = {
            ...
            print(f"[IndeedEuroWage] Successfully fetched CSV with {len(df)} rows")
            return df

        except requests.exceptions.RequestException as e:
            print(f"[IndeedEuroWage] Error fetching data: {e}")
            return None
        except Exception as e:
            print(f"[IndeedEuroWage] Error parsing CSV data: {e}")
            return None

    def _process_country_data(self, df: pd.DataFrame, country_code: str) -> List[Dict]:
        """
        特定の国のデータを処理

        Args:
            df: Full DataFrame
            country_code: Country code (DE, FR, EA)

        Returns:
            List of processed data points
        """
        # Filter by country code
        country_df = df[df["jobcountry"] == country_code].copy()

        if len(country_df) == 0:
            print(f"[IndeedEuroWage] No data found for country code: {country_code}")
            return []
        ...
        for _, row in country_df.iterrows():
            try:
                ...
                print(f"[IndeedEuroWage] Error processing row for {country_code}: {e}")
                continue

        # Sort by date
        result_data.sort(key=lambda x: x["date"])

        print(f"[IndeedEuroWage] Processed {len(result_data)} data points for {country_code}")
        return result_data
    ...
    def _is_data_updated(self, cached_data: Optional[Dict], new_data: List[Dict]) -> bool:
        ...
        cached_list = cached_data.get("euro_area", [])
        if not cached_list:
            return True
    ...
    def _set_monthly_updated(self) -> None:
        ...
        try:
            ...
            print(f"[IndeedEuroWage] Error setting monthly update flag: {e}")
    ...
    def _should_check_update(self) -> bool:
        ...
        try:
            ...
            print(f"[IndeedEuroWage] Error checking update schedule: {e}")
            return True
    ...
    def _set_last_check_time(self) -> None:
        ...
        try:
            ...
            print(f"[IndeedEuroWage] Error setting last check time: {e}")
    ...
    def _should_refresh(self, last_updated_str: str) -> bool:
        ...
        try:
            ...
            print(f"[IndeedEuroWage] Error checking refresh: {e}")
            return True
    ...
    def _load_file_cache(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            print(f"[IndeedEuroWage] Failed to load file cache: {e}")
            return None
    ...
    def _save_file_cache(self, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            print(f"[IndeedEuroWage] Cache saved to {DATA_CACHE_FILE}")
        except Exception as e:
            print(f"[IndeedEuroWage] Failed to save file cache: {e}")
    ...
    def get_cache_status(self) -> Dict[str, Any]:
        ...
        return {
            "indicator": "Indeed Euro Wage Tracker",
            "source": "Indeed Hiring Lab (GitHub)",
            "cache_key": self.DATA_CACHE_KEY,
            "exists": data_exists,
            "last_updated": cached_data.get("last_updated") if cached_data else None,
            "germany_count": len(cached_data.get("germany", [])) if cached_data else 0,
            "france_count": len(cached_data.get("france", [])) if cached_data else 0,
...
# シングルトンインスタンス
indeed_euro_wage_service = IndeedEuroWageService()
