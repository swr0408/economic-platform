"""
BOJ Rate Cuts Expectation Screenshot Service
Captures screenshots from MacroMicro charts via PlaywrightRunner

URLs (2026年):
- 年末金利予想: https://en.macromicro.me/series/78283/japan-boj-year-end-interest-rate-expectation-2026
- 利上げ・利下げ回数: https://en.macromicro.me/series/78289/japan-boj-interest-rate-cuts-expectation-2026
...
YEAREND_SCREENSHOT_FILENAME = "boj_yearend_rate_expectations.png"
RATE_CUTS_SCREENSHOT_FILENAME = "boj_rate_cuts_expectations.png"
...
YEAREND_URL = "https://en.macromicro.me/series/78283/japan-boj-year-end-interest-rate-expectation-2026"
RATE_CUTS_URL = "https://en.macromicro.me/series/78289/japan-boj-interest-rate-cuts-expectation-2026"


class BOJRateCutsScreenshotService:
    """BOJ Rate Cuts Expectation Screenshot Service"""

    CACHE_KEY = "japan:boj_rate_cuts_screenshot:metadata"
    ...
    def capture_all_screenshots(self, force_refresh: bool = False) -> Dict[str, Any]:
        ...
        for key, url, path, filename in items:
            if not force_refresh and path.exists():
                ...
                if file_age < 86400:
                    results[key] = {
                        "success": True,
                        "url": f"/cache/japan/policy/{filename}",
                        "cached": True,
                    }
                    logger.info(
                        f"[BOJRateCuts] Using cached {key} screenshot "
        ...
        for idx, (key, url, path, filename) in enumerate(to_capture):
            ...
            try:
                ...
                logger.error(f"[BOJRateCuts] {key} capture failed: {e}")
                results[key] = {
                    "success": False,
                    "url": None,
                    "cached": False,
    ...
    def get_screenshot_urls(self) -> Dict[str, Any]:
        ...
        if YEAREND_SCREENSHOT_PATH.exists():
            result["yearend_url"] = f"/cache/japan/policy/{YEAREND_SCREENSHOT_FILENAME}"

        if RATE_CUTS_SCREENSHOT_PATH.exists():
            result["rate_cuts_url"] = f"/cache/japan/policy/{RATE_CUTS_SCREENSHOT_FILENAME}"
    ...
    def get_cache_status(self) -> Dict[str, Any]:
        cached_meta = redis_client.get(self.CACHE_KEY)
        return {
            "indicator": "BOJ Rate Cuts Expectations Screenshots",
            "source": "MacroMicro",
...
boj_rate_cuts_screenshot_service = BOJRateCutsScreenshotService()
