...
DATA_CACHE_FILE = CACHE_DIR / "jp_pmi_cache.json"


class JapanPMIService:
    """日本 S&P Global PMIサービス"""

    DATA_CACHE_KEY = "japan:economy:jp_pmi:data"

    # 3つのPMI指標に対応するECONALPHA_ID
    ECONALPHA_IDS = {
        "manufacturing": "jp_sp_manufacturing_pmi",
        "services": "jp_sp_services_pmi",
        "composite": "jp_sp_composite_pmi",
    }

    # DBクエリ用のイベントパターン（国コードでフィルタリング）
    # S&P Global PMI と Jibun Bank PMI の両方を取得
    EVENT_PATTERNS = {
        "manufacturing": ["S&P Global Manufacturing PMI", "Jibun Bank Manufacturing PMI"],
        "services": ["S&P Global Services PMI", "Jibun Bank Services PMI"],
        "composite": ["S&P Global Composite PMI", "Jibun Bank Composite PMI"],
    }
    ...
    def get_jp_pmi_data(
        ...
        """
        日本 S&P Global PMIデータを取得（製造業、サービス業、総合）
    ...
    def _load_from_db(self, pmi_type: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            with SessionLocal() as session:
                # country='JP'でフィルタリングして他国のPMIを除外
                # 複数パターン（S&P Global と Jibun Bank）を OR で検索
                ...
                for row in rows:
                    ...
                    if dt_utc:
                        ...
                        if match:
                            ...
                            # 括弧内に月がない場合（CSV_IMPORTなど）はdatetime_utcの月をそのまま使用
                            # CSVインポートではdatetime_utcがデータの月に設定されている
                ...
                print(f"Loaded {len(result)} JP S&P {pmi_type.capitalize()} PMI records from DB")
                return result

        except Exception as e:
            print(f"Error loading JP S&P {pmi_type} PMI from DB: {e}")
            import traceback
            traceback.print_exc()
            return []
    ...
    def get_cache_status(self) -> Dict[str, Any]:
        ...
        return {
            "indicator": "JP S&P Global PMI (Manufacturing/Services/Composite)",
            "source": "Database (FMP)",
...
# シングルトンインスタンス
jp_pmi_service = JapanPMIService()
