...
BOJ (日本銀行) からCGPI ZIPファイル経由で輸入・輸出物価指数を取得

指標:
- 輸出物価指数（円ベース）: 総平均 - 前年比
- 輸入物価指数（円ベース）: 総平均 - 前年比
- 輸入物価指数（契約通貨ベース）: 総平均 - 前年比
...
DATA_CACHE_FILE = CACHE_DIR / "japan_import_export_price_cache.json"


class JapanImportExportPriceService:
    """日本輸入・輸出物価指数サービス"""

    DATA_CACHE_KEY = "japan:import_export_price:data"
    ...
    IMPORT_CONTRACT_PATTERN = "[輸入物価指数/契約通貨ベース] 総平均"
    ...
    def _fetch_from_boj(self) -> Optional[Dict[str, Any]]:
        """BOJ ZIPファイルからデータを取得"""
        try:
            print(f"Fetching Japan Import/Export Price data from BOJ ZIP: {self.CGPI_ZIP_URL}")
            ...
            print(f"Error fetching Import/Export Price data: {e}")
            import traceback
            traceback.print_exc()
            return None
    ...
    def _parse_csv(self, csv_content: str) -> Optional[List[Dict[str, Any]]]:
        ...
        抽出対象:
        - 輸出物価指数（円ベース）: [輸出物価指数/円ベース] 総平均
        - 輸入物価指数（円ベース）: [輸入物価指数/円ベース] 総平均
        - 輸入物価指数（契約通貨ベース）: [輸入物価指数/契約通貨ベース] 総平均
        """
        try:
            ...
            for line in lines[1:]:
                ...
                if self.IMPORT_CONTRACT_PATTERN in category_name and import_contract_row is None:
                    import_contract_row = parts
                    print(f"Found import contract row: {category_name}")

                if export_yen_row and import_yen_row and import_contract_row:
                    break

            if not export_yen_row and not import_yen_row and not import_contract_row:
            ...
            for idx, date_str in date_columns:
                ...
                if export_yen_row and idx < len(export_yen_row):
                ...
                if import_yen_row and idx < len(import_yen_row):
                ...
                if import_contract_row and idx < len(import_contract_row):
                    val_str = import_contract_row[idx].strip()
                    if val_str:
                        try:
                            import_contract_values[date_key] = float(val_str)
                        except ValueError:
                            pass

            # 前年比を計算してデータを構築
            all_dates = sorted(set(export_yen_values.keys()) | set(import_yen_values.keys()) | set(import_contract_values.keys()))
            series_data = []

            for date_key in all_dates:
                export_yen_index = export_yen_values.get(date_key)
                import_yen_index = import_yen_values.get(date_key)
                import_contract_index = import_contract_values.get(date_key)
                ...
                export_yen_yoy = None
                import_yen_yoy = None
                import_contract_yoy = None

                # 輸出物価（円ベース）の前年比
                if export_yen_index is not None:
                    export_prev = export_yen_values.get(year_ago_key)
                    if export_prev and export_prev != 0:
                        export_yen_yoy = round(((export_yen_index - export_prev) / export_prev) * 100, 2)

                # 輸入物価（円ベース）の前年比
                if import_yen_index is not None:
                    import_prev = import_yen_values.get(year_ago_key)
                    if import_prev and import_prev != 0:
                        import_yen_yoy = round(((import_yen_index - import_prev) / import_prev) * 100, 2)

                # 輸入物価（契約通貨ベース）の前年比
                if import_contract_index is not None:
                    import_contract_prev = import_contract_values.get(year_ago_key)
                    if import_contract_prev and import_contract_prev != 0:
                        import_contract_yoy = round(((import_contract_index - import_contract_prev) / import_contract_prev) * 100, 2)

                # いずれかの前年比がある場合のみ追加
                if export_yen_yoy is not None or import_yen_yoy is not None or import_contract_yoy is not None:
                    series_data.append({
                        "date": date_key,
                        "export_yen_yoy": export_yen_yoy,
                        "import_yen_yoy": import_yen_yoy,
                        "import_contract_yoy": import_contract_yoy,
                    })
            ...
            print(f"Processed Import/Export Price data: {len(series_data)} data points")
            if series_data:
    ...
    def _calculate_next_release(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return {
                ...
                "label": f"企業物価指数（輸入・輸出物価含む）- {next_release_date.strftime('%Y/%m/%d')} 8:50 JST（予定）"
            }
    ...
    def get_cache_status(self) -> Dict[str, Any]:
        ...
        return {
            "indicator": "Japan Import/Export Price Index",
...
# シングルトンインスタンス
japan_import_export_price_service = JapanImportExportPriceService()
