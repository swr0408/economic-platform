"""
日本 東京都区部消費者物価指数 (Tokyo CPI) サービス

e-Stat API から東京都区部CPI（Consumer Price Index）データを取得
...
DATA_CACHE_FILE = CACHE_DIR / "tokyo_cpi_cache.json"


class JapanTokyoCPIService:
    """日本東京都区部消費者物価指数サービス"""

    DATA_CACHE_KEY = "japan:tokyo_cpi:data"
    ECONALPHA_ID = "jp_tokyo_cpi"
    ...
    def _fetch_from_estat(self) -> Optional[Dict[str, Any]]:
        """e-Stat APIから東京都区部CPIデータを取得"""
        if not self.api_key:
            print("e-Stat API key not found")
            return None

        try:
            print("Fetching Japan Tokyo CPI data from e-Stat API")
            ...
            if not all_items_yoy:
                print("Failed to fetch Tokyo CPI data")
                return None
            ...
            print(f"Error fetching Tokyo CPI data: {e}")
            import traceback
            traceback.print_exc()
            return None
    ...
    def _fetch_seasonally_adjusted_mom(self) -> Optional[Dict[str, Dict[str, float]]]:
        """
        e-Stat CSVから季節調整済み前月比データを取得（東京都区部）
        ...
        """
        try:
            print("Fetching Tokyo seasonally adjusted MoM data from e-Stat CSV")
            ...
            if response.status_code != 200:
                print(f"Failed to fetch Tokyo CSV: status {response.status_code}")
                return None
            ...
            print(f"Fetched {len(result)} Tokyo seasonally adjusted MoM data points")
            return result

        except Exception as e:
            print(f"Error fetching Tokyo seasonally adjusted MoM: {e}")
            import traceback
            traceback.print_exc()
            return None
    ...
    def _fetch_cpi_series(
        ...
        try:
            params = {
                ...
                "cdTab": tab_code,
                "cdArea": self.TOKYO_AREA_CODE,  # 東京都区部
                "metaGetFlg": "N",
            ...
            print(f"Fetched {len(parsed_data)} Tokyo CPI data points for category {category_code}")
            return parsed_data

        except Exception as e:
            print(f"Error fetching Tokyo CPI series {category_code}: {e}")
            return None
...
# シングルトンインスタンス
japan_tokyo_cpi_service = JapanTokyoCPIService()
