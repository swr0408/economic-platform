"""
非農業部門雇用者数（Nonfarm Payrolls）サービス
FRED APIから雇用者数データを取得

指標:
- PAYEMS: 非農業部門雇用者数（All Employees, Total Nonfarm）
- CE16OV: 民間雇用者数（Civilian Employment Level）
...
DATA_CACHE_FILE = CACHE_DIR / "nonfarm_payrolls_cache.json"


class NonfarmPayrollsService:
    """非農業部門雇用者数サービス"""

    BASE_URL = "https://api.stlouisfed.org/fred"
    DATA_CACHE_KEY = "fred:nonfarm_payrolls:data"
    ECONALPHA_ID = "nonfarm_payrolls"  # FMPマッピング用ID
    ...
    def get_nonfarm_payrolls_data(
        ...
        Returns:
            {
                "data": [{"date": str, "nonfarm": float, "civilian": float}, ...],
                "latest": {...},
                "series_config": {...},
            ...
            if cached_data:
                ...
                if last_updated_str and not self._should_refresh(last_updated_str):
                    return {
                        ...
                        "next_release": get_next_release_from_fmp('nonfarm_payrolls'),
                        "cached": True,
                        "source": "redis",
    ...
    def _fetch_from_api(self, start_date: Optional[str] = None) -> Optional[List[Dict[str, Any]]]:
        ...
        try:
            ...
            print(f"Fetched {len(merged_data)} Nonfarm Payrolls records")
            return merged_data

        except Exception as e:
            print(f"Error fetching Nonfarm Payrolls: {e}")
            import traceback
            traceback.print_exc()
            return None
...
# シングルトンインスタンス
nonfarm_payrolls_service = NonfarmPayrollsService()
