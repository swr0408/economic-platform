"""
ISM非製造業景況指数サービス
DBからISM Non-Manufacturing PMI（Services PMI）データを取得

指標:
- ISM Services PMI: ISM非製造業購買担当者景気指数
...
DATA_CACHE_FILE = CACHE_DIR / "ism_non_manufacturing_cache.json"


class ISMNonManufacturingService:
    """ISM非製造業景況指数サービス"""

    DATA_CACHE_KEY = "ism:non_manufacturing:data"
    ECONALPHA_ID = "ism_non_manufacturing"
    ...
    def get_ism_non_manufacturing_data(
        ...
        if db_result:
            ...
            # 発表レース対策（ラグガード）:
            # ISM発表(第3営業日 10:00 ET)直後～FMPカレンダー同期(最大1時間ラグ)の間に
            ...
            # 維持し、次回リクエストで再取得して新月を自己回復させる。（製造業側と同ロジック）
            ...
            if existing_latest_date and new_latest_date and new_latest_date <= existing_latest_date:
                ...
                print(f"  ISM Non-Manufacturing: DB not newer (latest={new_latest_date}), keeping last_updated={last_updated}")
            else:
        ...
        try:
            ...
            with SessionLocal() as session:
                ...
                for row in rows:
                    ...
                    if dt_utc:
                        # イベント名から対象月を抽出（例: "ISM Services PMI (Dec)"）
    ...
    def get_cache_status(self) -> Dict[str, Any]:
        ...
        return {
            "indicator": "ISM Non-Manufacturing PMI (Services)",
            "source": "Database (FMP)",
