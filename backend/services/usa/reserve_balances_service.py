"""
準備預金残高（Reserve Balances）サービス
FREDからWRBWFRBLシリーズを取得

指標:
- Reserve Balances with Federal Reserve Banks（準備預金残高）

データソース:
- FRED: WRBWFRBL（Reserve Balances with Federal Reserve Banks: Week Average）
...
class ReserveBalancesService(BaseSingleSeriesService):
    """準備預金残高サービス（FRED WRBWFRBL）"""

    SERIES_ID = "WRBWFRBL"
    REDIS_KEY = "fred:reserve_balances:data"
    ECONALPHA_ID = ""  # FMPマッピングなし
    CACHE_FILE = CACHE_DIR / "reserve_balances_cache.json"
    INDICATOR_NAME = "Reserve Balances"
    ...
    def get_data(
        ...
        # 2. APIから取得
        api_data = self._fetch_series(start_date)
        ...
        if api_data:
            ...
            # Redisに保存
            redis_client.set(self.REDIS_KEY, cache_payload)
    ...
    def _fetch_series(self, start_date: Optional[str] = None) -> List[Dict[str, Any]]:
        """FREDからシリーズを取得"""
        try:
...
# シングルトンインスタンス
reserve_balances_service = ReserveBalancesService()
