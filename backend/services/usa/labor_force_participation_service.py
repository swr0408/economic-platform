...
# FREDシリーズID
CIVPART_SERIES_ID = "CIVPART"  # 労働参加率

# キャッシュディレクトリ
CACHE_DIR = Path(__file__).parent.parent.parent / "cache" / "usa" / "employment"
CACHE_DIR.mkdir(parents=True, exist_ok=True)
DATA_CACHE_FILE = CACHE_DIR / "labor_force_participation_cache.json"


class LaborForceParticipationService:
    """労働参加率サービス"""

    BASE_URL = "https://api.stlouisfed.org/fred"
    DATA_CACHE_KEY = "fred:labor_force_participation:data"
    ...
    def get_labor_force_participation_data(
        self,
    ...
    def _fetch_from_api(self, start_date: Optional[str] = None) -> Optional[List[Dict[str, Any]]]:
        ...
        try:
            ...
            # 労働参加率（水準値）
            civpart_data = self._fetch_series(CIVPART_SERIES_ID, start_date)

            if not civpart_data:
                return None

            print(f"Fetched {len(civpart_data)} Labor Force Participation records")
            return civpart_data

        except Exception as e:
            print(f"Error fetching Labor Force Participation: {e}")
            import traceback
            traceback.print_exc()
            return None
    ...
    def _get_next_release(self) -> Optional[Dict[str, Any]]:
        ...
        労働参加率はEmployment Situation（雇用統計）と同時発表
        """
        try:
            # 失業率サービスから次回発表日を取得
            ...
            if next_release:
                return {
                    ...
                    "label": next_release.get("label", "").replace("Employment Situation", "Labor Force Participation")
                }
