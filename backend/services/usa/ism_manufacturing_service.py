...
class ISMManufacturingService:
    ...
    def _should_refresh(self, last_updated_str: str, next_release: Optional[Dict]) -> bool:
        ...
        try:
            ...
            if not next_release or not next_release.get("date"):
                # 次回発表日がない場合は24時間後に更新
                return datetime.now(JST) > last_updated + timedelta(hours=24)

            next_release_date = datetime.strptime(next_release["date"], "%Y-%m-%d")
            # ISM発表時刻: 通常10:00 ET = 0:00 JST（翌日）+ バッファ
            next_release_datetime = next_release_date.replace(
                hour=1, minute=0, tzinfo=JST
            ) + timedelta(days=1)

            now = datetime.now(JST)

            # 発表日時を過ぎていて、かつ最終更新が発表日時より前なら更新必要
            if now >= next_release_datetime and last_updated < next_release_datetime:
                return True
