...
CSV_FILE = CSV_DIR / "ism_non_manufacturing_components.csv"
...
class ISMNonManufacturingComponentsService:
        ...
        try:
            ...
            with open(CSV_FILE, "r", encoding="utf-8") as f:
                ...
                for row in reader:
                    ...
                    for field in ["new_orders", "business_activity", "employment",
            ...
            for date_key, csv_fields in csv_data.items():
                if date_key in date_index:
                    ...
                    entry = {
                        ...
                        "business_activity": csv_fields.get("business_activity"),
    ...
    def _save_to_db(self, data: List[Dict[str, Any]], source: str = "dbnomics") -> None:
        ...
        try:
            ...
            count = upsert_ism_components("non_manufacturing", data, source=source)
            if count > 0:
    ...
    def _save_csv_to_db(self, data: List[Dict[str, Any]]) -> None:
        ...
        try:
            ...
            if csv_records:
                from services.usa.ism_components_db_utils import upsert_ism_components
                count = upsert_ism_components("non_manufacturing", csv_records, source="csv")
                if count > 0:
    ...
    def _write_fmp_to_csv(self, data: List[Dict[str, Any]]) -> None:
        ...
        try:
            ...
            db_latest = get_latest_date("non_manufacturing", source="dbnomics")
            if not db_latest:
                return
            ...
            CSV_FIELDS = ["new_orders", "business_activity", "employment",
                          "supplier_deliveries", "prices", "inventories"]
    ...
    def _load_from_db(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            db_data = load_ism_components("non_manufacturing")
            return db_data
        except Exception as e:
    ...
    def _supplement_from_fmp_db(self, dbnomics_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        FMP DBにはNew Orders, Business Activity, Employment, Pricesが含まれる。
        ...
        try:
            ...
            # FMP DBからISM非製造業コンポーネントを取得（DBnomics最新以降）
            # FMPでは "ISM Non-Manufacturing" または "ISM Services" の2パターンがある
            fmp_components = {
                "new_orders": ["ISM Non-Manufacturing New Orders", "ISM Services New Orders"],
                "business_activity": ["ISM Non-Manufacturing Business Activity", "ISM Services Business Activity"],
                "employment": ["ISM Non-Manufacturing Employment", "ISM Services Employment"],
                "prices": ["ISM Non-Manufacturing Prices", "ISM Services Prices"],
            }

            with SessionLocal() as session:
                new_months = {}
                for field, patterns in fmp_components.items():
                    for pattern in patterns:
                        ...
                        for row in rows:
                            event_name = row[0]
                            actual = float(row[2])
                            # イベント名から月を抽出: "ISM Services New Orders (Feb)" → Feb
                            import re
                ...
                for date_key, new_data in sorted(new_months.items()):
                    ...
                    entry = {
                        "date": date_key,
                        "new_orders": new_orders,
                        "business_activity": new_data.get("business_activity"),
                    ...
                    print(f"  Supplemented from FMP DB: {date_key} (new_orders={new_orders}, business_activity={new_data.get('business_activity')}, employment={new_data.get('employment')}, prices={new_data.get('prices')})")
