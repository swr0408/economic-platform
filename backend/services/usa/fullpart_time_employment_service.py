"""
フルタイム/パートタイム雇用者数サービス
FRED APIから雇用形態別雇用者数データを取得

指標:
- LNS12500000: フルタイム雇用者数（Employed Full Time）
- LNS12600000: パートタイム雇用者数（Employed Part Time）
...
SERIES_CONFIG = {
    "fulltime": {
        "series_id": "LNS12500000",
        "name": "フルタイム",
        "name_en": "Employed Full Time",
        "color": "#1890ff"
    },
    "parttime": {
        "series_id": "LNS12600000",
        "name": "パートタイム",
        "name_en": "Employed Part Time",
        "color": "#52c41a"
    }
}
...
DATA_CACHE_FILE = CACHE_DIR / "fullpart_time_employment_cache.json"


class FullPartTimeEmploymentService:
    """フルタイム/パートタイム雇用者数サービス"""

    BASE_URL = "https://api.stlouisfed.org/fred"
    DATA_CACHE_KEY = "fred:fullpart_time_employment:data"
    ...
    def get_fullpart_time_employment_data(
        ...
        Returns:
            {
                "data": [{"date": str, "fulltime": float, "parttime": float}, ...],
                "latest": {...},
                "series_config": {...},
    ...
    def _fetch_from_api(self, start_date: Optional[str] = None) -> Optional[List[Dict[str, Any]]]:
        """FRED APIからフルタイム/パートタイム雇用者数データを取得"""
        try:
            if not self.api_key:
                print("FRED_API_KEY not set")
                return None

            print("Fetching Full/Part-Time Employment from FRED...")
            ...
            print(f"Fetched {len(merged_data)} Full/Part-Time Employment records")
            return merged_data

        except Exception as e:
            print(f"Error fetching Full/Part-Time Employment: {e}")
            import traceback
            traceback.print_exc()
            return None
    ...
    def get_cache_status(self) -> Dict[str, Any]:
        ...
        return {
            "indicator": "Full/Part-Time Employment",
            "source": "FRED / BLS",
...
# シングルトンインスタンス
fullpart_time_employment_service = FullPartTimeEmploymentService()
