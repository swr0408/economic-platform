"""
TGA（Treasury General Account）サービス
FREDからWDTGALシリーズを取得

指標:
- Treasury General Account（財務省一般勘定残高）

データソース:
- FRED: WDTGAL（Millions of Dollars）
...
class TGAService(BaseSingleSeriesService):
    """TGA（Treasury General Account）サービス（FRED WDTGAL）"""

    SERIES_ID = "WDTGAL"
    REDIS_KEY = "fred:tga:data"
    ECONALPHA_ID = ""  # FMPマッピングなし
    CACHE_FILE = CACHE_DIR / "tga_cache.json"
    INDICATOR_NAME = "TGA"
