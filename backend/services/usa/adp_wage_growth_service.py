...
class ADPWageGrowthService:
        ...
        try:
            ...
            if next_release and next_release.get("date"):
                ...
                if now >= release_jst and last_updated < release_jst:
                    print(f"[ADP Wage] Release time passed: {release_jst}, last_updated: {last_updated}")
                    return True
    ...
    def _get_next_release(self) -> Optional[Dict[str, Any]]:
        """
        次回発表日を取得（ADP雇用者数サービスの次回発表日を参照）
        ADP Pay InsightsはADP National Employment Reportと同時発表
        """
        try:
            # ADP雇用者数サービスから次回発表日を取得
            ...
            if next_release:
                return {
                    ...
                    "label": next_release.get("label", "").replace("National Employment Report", "Pay Insights")
                }
