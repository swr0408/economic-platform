...
使用方法:
    ...
    # パターンを指定して次回発表日を取得
    next_release = get_next_release_by_pattern('RBNZ Interest Rate Decision', country='NZ')
"""
...
def get_next_release_by_pattern(
    ...
    Args:
        event_pattern: イベント名のパターン（例: "RBNZ Interest Rate Decision"）
        use_cache: Redisキャッシュを使用するか
        country: 国コード（デフォルト: "NZ"）
    ...
    try:
        ...
        for event in events:
            ...
            if dt_utc and dt_utc.date() >= today:
                ...
                candidates.append({
                    ...
                    "datetime_auckland": dt_auckland.isoformat(),
                    "time_auckland": dt_auckland.strftime("%H:%M"),
                    "label": event_name,
        ...
        print(f"[NZ FMP Utils] Error fetching next release by pattern '{event_pattern}' for country '{country}': {e}")
        return None
...
def _get_next_release_from_db(
    ...
    try:
        ...
        print(f"[NZ FMP Utils] Error fetching from DB: {e}")
        return None
...
def should_refresh_by_pattern(
    ...
    try:
        ...
        # 発表時刻ちょうどの再取得でソース(Stats NZ/RBNZ等)未反映のまま last_updated=now を
        # 刻むと、下のスケジュール判定が「発表消化済み」と誤認し次回発表まで永久凍結する
        # （NZ utils は従来 max-age を持たず、四半期指標では最大3ヶ月凍結した）。
        ...
        print(f"[NZ FMP Utils] Error checking refresh by pattern '{event_pattern}' for country '{country}': {e}")
        return False


def invalidate_next_release_cache(event_pattern: str, country: str = "NZ") -> bool:
    """次回発表日キャッシュを無効化"""
    cache_key = f"fmp:next_release:pattern:{country}:{event_pattern.lower().replace(' ', '_')}"
