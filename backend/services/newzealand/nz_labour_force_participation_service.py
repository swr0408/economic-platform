...
指標:
- Labour Force Participation Rate（労働参加率）- HLFQ.S1E3S（季節調整済み、Total）
...
- Table 1 の「Total」セクション（Row 35付近）から労働参加率（Col 13 = S1E3S）を取得
- 各Excelには約9四半期分のデータのみ含まれるため、過去のExcelも取得して長期データを構築

系列ID:
- S1E3S : Labour Force Participation Rate (SA) (%)
...
DATA_CACHE_FILE = CACHE_DIR / "nz_labour_force_participation_cache.json"
...
def _parse_next_release_from_contents(excel_bytes: bytes) -> Optional[Dict[str, Any]]:
    ...
    try:
            ...
            if "will be published" in val.lower():
                ...
                if match:
                    ...
                    try:
                        ...
                        print(f"[NzParticipation] Next release parsed: {dt.strftime('%Y-%m-%d')}")
                        return {
        ...
        print(f"[NzParticipation] Error parsing Contents for next release: {e}")
        return None


class NzLabourForceParticipationService:
    """NZ 労働参加率サービス"""

    DATA_CACHE_KEY = "newzealand:nz_labour_force_participation:data"
    ...
    def get_data(self, force_refresh: bool = False) -> Dict[str, Any]:
        ...
        if result:
            ...
            cache_payload = {
                ...
                "metadata": {
                    "source": "Stats NZ",
                    "indicator": "Household Labour Force Survey - Participation Rate",
                    "description": "NZ 労働参加率（季節調整済み）",
                    "unit": "%",
                    "frequency": "quarterly",
                    "series": "HLFQ.S1E3S",
                },
    ...
    def _download_excel(self, year: int, qm: int) -> Optional[bytes]:
        ...
        try:
            ...
            if resp.status_code == 200 and len(resp.content) > 50000:
                ...
                print(f"[NzParticipation] Downloaded {month_cap} {year} ({len(resp.content)} bytes)")
                return resp.content
        except Exception as e:
            print(f"[NzParticipation] Error downloading {year} Q{qm // 3}: {e}")
        return None

    def _load_from_stats_nz(self) -> Tuple[Optional[List[Dict[str, Any]]], Optional[Dict[str, Any]]]:
        """Stats NZ HLFS Excelから労働参加率データを取得
        ...
        try:
            ...
            if latest_excel is None:
                print("[NzParticipation] Failed to download latest HLFS Excel")
                return None, None

            # 2. 最新Excelから労働参加率データ取得
            all_data: Dict[str, Dict[str, Any]] = {}
            latest_data = self._parse_table1_participation(latest_excel)
            ...
            for hist_year, hist_qm in historical_quarters:
                ...
                hist_data = self._parse_table1_participation(hist_excel)
            ...
            if result:
                print(f"[NzParticipation] Total {len(result)} records")
                print(f"[NzParticipation] Date range: {result[0]['date']} to {result[-1]['date']}")
                print(f"[NzParticipation] Latest: {result[-1]['value']}%")
            if next_release:
                print(f"[NzParticipation] Next release: {next_release.get('date')}")

            return result, next_release

        except Exception as e:
            print(f"[NzParticipation] Error loading from Stats NZ: {e}")
            import traceback
            traceback.print_exc()
            return None, None

    def _parse_table1_participation(self, excel_bytes: bytes) -> Dict[str, Dict[str, Any]]:
        """Table 1 の「Total」セクションから労働参加率を取得

        Table 1 構造:
        - Row 34付近: "Total" ラベル
        - Row 35: Series ref（S1E3S が Col 12）
        - Row 36: "Quarter" ヘッダー
        - Row 37+: データ行（Year Col0, Quarter Col1, Participation Rate Col12）
        """
        try:
            ...
            if total_row is None:
                print("[NzParticipation] 'Total' section not found in Table 1")
                return {}
            ...
            for row_idx in range(data_start, df.shape[0]):
                ...
                # 労働参加率（Col 12 = S1E3S）
                part_val = df.iloc[row_idx, 12] if df.shape[1] > 12 else None
                if pd.isna(part_val):
                    continue
                try:
                    value = round(float(part_val), 1)
                except (ValueError, TypeError):
                    continue

                data[date_str] = {"value": value}

            print(f"[NzParticipation] Table 1 Total: {len(data)} records")
            return data

        except Exception as e:
            print(f"[NzParticipation] Error parsing Table 1: {e}")
            import traceback
            traceback.print_exc()
            return {}
    ...
    def _load_file_cache(self) -> Optional[Dict[str, Any]]:
        try:
            ...
            logger.error(f"[NzParticipation] Failed to load file cache: {e}")
            return None
    ...
    def _save_file_cache(self, data: Dict[str, Any]) -> None:
        try:
            ...
            logger.error(f"[NzParticipation] Failed to save file cache: {e}")
    ...
    def get_cache_status(self) -> Dict[str, Any]:
        ...
        return {
            "indicator": "NZ Labour Force Participation Rate (HLFS)",
            "source": "Stats NZ",
...
# シングルトンインスタンス
nz_labour_force_participation_service = NzLabourForceParticipationService()
