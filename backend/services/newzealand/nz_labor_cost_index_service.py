...
- Labour Cost Index（労働コスト指数）- LCIQ.SG53Z9
  All salary and wage rates (incl overtime) index (Base: Jun 2009 = 1000)
...
取得方法:
- Stats NZ リリースページから LCI Excel を直接ダウンロード
- Table 1 の Block 3（All salary and wage rates including overtime）から SG53Z9 を取得
- インデックス値 / QoQ% / YoY% の3セクションを解析
- 各Excelには約5四半期分のデータのみ含まれるため、過去のExcelも取得して長期データを構築
...
DATA_CACHE_FILE = CACHE_DIR / "nz_labor_cost_index_cache.json"
...
def _parse_next_release_from_contents(excel_bytes: bytes) -> Optional[Dict[str, Any]]:
    ...
    try:
            ...
            if "will be published" in val.lower() or "will be released" in val.lower():
                ...
                if match:
                    ...
                    try:
                        ...
                        print(f"[NzLCI] Next release parsed: {dt.strftime('%Y-%m-%d')}")
                        return {
        ...
        print(f"[NzLCI] Error parsing Contents for next release: {e}")
        return None


class NzLaborCostIndexService:
    """NZ 労働コスト指数サービス"""

    DATA_CACHE_KEY = "newzealand:nz_labor_cost_index:data"
    ...
    def get_data(self, force_refresh: bool = False) -> Dict[str, Any]:
        ...
        if result:
            ...
            cache_payload = {
                ...
                "metadata": {
                    "source": "Stats NZ",
                    "indicator": "Labour Cost Index - All salary and wage rates",
                    "description": "NZ 労働コスト指数（全給与・賃金率 残業込み）",
                    "unit": "index (Base: Jun 2009 = 1000)",
                    "frequency": "quarterly",
                    "series": "LCIQ.SG53Z9",
                },
    ...
    def _load_from_stats_nz(self) -> Tuple[Optional[List[Dict[str, Any]]], Optional[Dict[str, Any]]]:
        ...
        try:
            ...
            if latest_excel is None:
                print("[NzLCI] Failed to download latest LCI Excel")
                return None, None
            ...
            if result:
                print(f"[NzLCI] Total {len(result)} records")
                print(f"[NzLCI] Date range: {result[0]['date']} to {result[-1]['date']}")
                latest = result[-1]
                if latest.get("qoq") is not None:
                    print(f"[NzLCI] Latest QoQ: {latest['qoq']}%")
                if latest.get("yoy") is not None:
                    print(f"[NzLCI] Latest YoY: {latest['yoy']}%")
            if next_release:
                print(f"[NzLCI] Next release: {next_release.get('date')}")

            return result, next_release

        except Exception as e:
            print(f"[NzLCI] Error loading from Stats NZ: {e}")
            import traceback
            traceback.print_exc()
            return None, None
    ...
    def _parse_lci_table1(self, excel_bytes: bytes) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            if sg53_row is None:
                print("[NzLCI] SG53Z9 not found in LCI Table 1")
                return {}

            # インデックス値セクション: sg53_row + 3 から開始
            index_data = self._parse_lci_section(df, sg53_row + 3)

            # QoQ%セクション: "Percentage change from previous quarter" の行を探す
            qoq_start = None
            ...
            # YoY%セクション: "Percentage change from previous year" の行を探す
            yoy_start = None
            ...
            print(f"[NzLCI] LCI Table 1: {len(data)} records parsed")
            return data

        except Exception as e:
            print(f"[NzLCI] Error parsing LCI Table 1: {e}")
            import traceback
            traceback.print_exc()
            return {}
    ...
    def _load_file_cache(self) -> Optional[Dict[str, Any]]:
        try:
            ...
            logger.error(f"[NzLCI] Failed to load file cache: {e}")
            return None
    ...
    def _save_file_cache(self, data: Dict[str, Any]) -> None:
        try:
            ...
            logger.error(f"[NzLCI] Failed to save file cache: {e}")
    ...
    def get_cache_status(self) -> Dict[str, Any]:
        ...
        return {
            "indicator": "NZ Labour Cost Index (LCI)",
            "source": "Stats NZ",
...
# シングルトンインスタンス
nz_labor_cost_index_service = NzLaborCostIndexService()
