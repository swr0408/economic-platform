...
class DistillateFuelInventoriesService:
    ...
    def _build_data(self) -> Optional[Dict[str, Any]]:
        """EIA API v2からデータを取得"""
        logger.info("[DistillateFuel] Building data from EIA API v2...")

        api_key = os.environ.get("EIA_API_KEY", "")
        if not api_key:
            logger.warning("[DistillateFuel] EIA_API_KEY not set")
            return None
        ...
        if not result_data:
            logger.error("[DistillateFuel] No data from EIA API")
            return None
        ...
        logger.info(
            f"[DistillateFuel] {len(result_data)} data points "
        ...
        while True:
            try:
                ...
                if resp.status_code != 200:
                    logger.error(f"[DistillateFuel] EIA API error: HTTP {resp.status_code}")
                ...
                logger.error(f"[DistillateFuel] EIA API error: {e}")
                return all_rows if all_rows else None

        logger.info(f"[DistillateFuel] Total rows fetched: {len(all_rows)}")
        return all_rows
