...
# スクリーンショットファイル名
YEAREND_SCREENSHOT_FILENAME = "boc_yearend_rate_expectations.png"
RATE_CUTS_SCREENSHOT_FILENAME = "boc_rate_cuts_expectations.png"
...
# スクリーンショット対象URL（2026年版 - BOC専用ページ）
...
class BocRateCutsScreenshotService:
    ...
    def get_screenshot_urls(self) -> Dict[str, Any]:
        ...
        if YEAREND_SCREENSHOT_PATH.exists():
            result["yearend_url"] = f"/cache/canada/policy/{YEAREND_SCREENSHOT_FILENAME}"

        if RATE_CUTS_SCREENSHOT_PATH.exists():
            result["rate_cuts_url"] = f"/cache/canada/policy/{RATE_CUTS_SCREENSHOT_FILENAME}"
    ...
    def get_cache_status(self) -> Dict[str, Any]:
        ...
        return {
            "indicator": "BOC Rate Cuts Expectations Screenshots",
            "source": "MacroMicro",
...
# シングルトンインスタンス
boc_rate_cuts_screenshot_service = BocRateCutsScreenshotService()
