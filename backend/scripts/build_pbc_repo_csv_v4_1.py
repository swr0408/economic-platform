"""
PBOC v4.1 - ログファイル出力 + スキップ理由記録追加
出力: pbc_reverse_repo.csv, pbc_scrape_log.txt, pbc_skipped.csv
"""
import re, time, csv
from urllib.parse import urljoin
import requests
from bs4 import BeautifulSoup

BASE = "https://www.pbc.gov.cn/zhengcehuobisi/125207/125213/125431/125475/"
PBC_ROOT = "https://www.pbc.gov.cn"
MAX_PAGE = 183
MIN_DATE = "2015-01-01"  # これより古い公告はスキップ（変更可）
OUT_CSV = "pbc_reverse_repo.csv"
LOG_FILE = "pbc_scrape_log.txt"
SKIP_CSV = "pbc_skipped.csv"
SLEEP = 0.3
ONLY_7D = False
MAX_RETRY = 3

HEADERS = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"}

log_lines = []
def log(msg):
    print(msg)
    log_lines.append(msg)

def list_url(page):
    return urljoin(BASE, "index.html") if page == 1 else urljoin(BASE, f"17081-{page}.html")

re_date = re.compile(r"(\d{4}-\d{2}-\d{2})")
re_detail_path = re.compile(r"/125475/\d{5,}/")
re_num = re.compile(r"([\d.]+)")

def fetch(url, retries=MAX_RETRY):
    for attempt in range(retries):
        try:
            r = requests.get(url, headers=HEADERS, timeout=30)
            r.raise_for_status()
            r.encoding = r.apparent_encoding
            time.sleep(SLEEP)
            return r.text
        except (requests.exceptions.Timeout, requests.exceptions.ConnectionError) as e:
            if attempt < retries - 1:
                wait = (attempt + 1) * 5
                log(f"    リトライ {attempt+1}/{retries} ({wait}秒待機)")
                time.sleep(wait)
            else:
                raise

def parse_list_page(html):
    soup = BeautifulSoup(html, "html.parser")
    items = []
    for a in soup.find_all("a", href=True):
        href = a["href"]
        if not re_detail_path.search(href):
            continue
        td = a.find_parent("td")
        if not td:
            continue
        span = td.find("span", class_="hui12")
        if span:
            m = re_date.search(span.get_text(strip=True))
        else:
            m = re_date.search(td.get_text(" ", strip=True))
        if not m:
            continue
        dt = m.group(1)
        title = a.get_text(strip=True)
        detail_url = urljoin(PBC_ROOT, href)
        items.append((detail_url, dt, title))
    return items

def extract_number(text):
    m = re_num.search(text.replace(",", "").replace("，", ""))
    return float(m.group(1)) if m else None

def parse_detail(html, dt):
    """戻り値: (dt, rate, bid, win) or (None, skip_reason)"""
    soup = BeautifulSoup(html, "html.parser")
    content = soup.find("div", id="zoom") or soup.find("div", class_="zoom1") or soup
    full_text = content.get_text("", strip=True)

    if "逆回购" not in full_text:
        return None, "逆回购なし"
    if "不开展逆回购" in full_text or "不开展 逆回购" in full_text:
        return None, "不开展逆回购"

    tables = content.find_all("table")
    for tbl in tables:
        rows = tbl.find_all("tr")
        if len(rows) < 2:
            continue
        header_cells = [td.get_text(strip=True) for td in rows[0].find_all(["td", "th"])]
        if not header_cells:
            continue
        col_map = {}
        for i, h in enumerate(header_cells):
            h_clean = h.replace("\n", "").replace(" ", "")
            if "期限" in h_clean:
                col_map["tenor"] = i
            elif "操作量" in h_clean or "投标量" in h_clean:
                col_map["bid"] = i
            elif "中标量" in h_clean or "交易量" in h_clean:
                col_map["win"] = i
            elif "操作利率" in h_clean or "中标利率" in h_clean:
                col_map["rate"] = i
        if "rate" not in col_map:
            continue
        for data_row in rows[1:]:
            cells = [td.get_text(strip=True) for td in data_row.find_all(["td", "th"])]
            if len(cells) <= max(col_map.values()):
                continue
            tenor_text = cells[col_map["tenor"]] if "tenor" in col_map else ""
            tenor_num = extract_number(tenor_text)
            if tenor_num is not None and ONLY_7D and int(tenor_num) != 7:
                continue
            rate = extract_number(cells[col_map["rate"]])
            if rate is None:
                continue
            bid, win = "", ""
            if "bid" in col_map and "win" in col_map:
                bid = extract_number(cells[col_map["bid"]]) or ""
                win = extract_number(cells[col_map["win"]]) or ""
            elif "bid" in col_map:
                val = extract_number(cells[col_map["bid"]])
                bid = val or ""
                win = val or ""
            elif "win" in col_map:
                win = extract_number(cells[col_map["win"]]) or ""
            return (dt, rate, bid, win), None

    compact = re.sub(r'\s+', '', full_text)
    m_intro = re.search(r'开展了([\d.]+)亿元(\d+)天期?逆回购', compact)
    if m_intro:
        win = float(m_intro.group(1))
        tenor = int(m_intro.group(2))
        if ONLY_7D and tenor != 7:
            return None, f"期限{tenor}天(7天のみモード)"
        rate_m = re.search(r'([\d.]+)%', compact)
        if rate_m:
            rate = float(rate_m.group(1))
            return (dt, rate, win, win), None

    return None, "テーブル解析失敗"

def main():
    rows = []
    skipped = []  # (dt, title, url, reason)
    seen = set()
    err_count = 0

    for page in range(1, MAX_PAGE + 1):
        url = list_url(page)
        log(f"[{page}/{MAX_PAGE}] {url}")
        try:
            html = fetch(url)
        except Exception as e:
            log(f"  ⚠ 一覧取得失敗: {e}")
            continue

        page_items = parse_list_page(html)
        if not page_items:
            log(f"  ⚠ リンク0件")

        # 全件がMIN_DATEより古ければ以降のページも古いので終了
        if page_items and all(dt < MIN_DATE for _, dt, _ in page_items):
            log(f"  ■ 全件が{MIN_DATE}以前 → 終了")
            break

        for detail_url, dt, title in page_items:
            if dt < MIN_DATE:
                continue
            if detail_url in seen:
                continue
            seen.add(detail_url)
            try:
                dhtml = fetch(detail_url)
                parsed, reason = parse_detail(dhtml, dt)
                if parsed:
                    rows.append(parsed)
                    log(f"  ✓ {dt} rate={parsed[1]} bid={parsed[2]} win={parsed[3]}")
                else:
                    skipped.append((dt, title, detail_url, reason))
            except Exception as e:
                err_count += 1
                skipped.append((dt, title, detail_url, f"エラー: {e}"))
                if err_count <= 5:
                    log(f"  ⚠ {dt} 詳細取得失敗: {e}")

    rows.sort(key=lambda x: x[0], reverse=True)

    with open(OUT_CSV, "w", newline="", encoding="utf-8-sig") as f:
        w = csv.writer(f)
        w.writerow(["日付", "利率(%)", "投标量(億元)", "中标量(億元)"])
        w.writerows(rows)

    # スキップCSV
    with open(SKIP_CSV, "w", newline="", encoding="utf-8-sig") as f:
        w = csv.writer(f)
        w.writerow(["日付", "タイトル", "URL", "スキップ理由"])
        w.writerows(skipped)

    # サマリー
    reasons = {}
    for _, _, _, r in skipped:
        reasons[r] = reasons.get(r, 0) + 1

    summary = f"\n{'='*50}\n完了: {OUT_CSV} ({len(rows)}行)\nスキップ: {len(skipped)}件, エラー: {err_count}件\n"
    summary += "\nスキップ理由の内訳:\n"
    for r, cnt in sorted(reasons.items(), key=lambda x: -x[1]):
        summary += f"  {r}: {cnt}件\n"
    summary += f"\nスキップ詳細: {SKIP_CSV}\n"

    log(summary)

    # ログファイル
    with open(LOG_FILE, "w", encoding="utf-8") as f:
        f.write("\n".join(log_lines))
    print(f"ログ: {LOG_FILE}")

if __name__ == "__main__":
    main()
